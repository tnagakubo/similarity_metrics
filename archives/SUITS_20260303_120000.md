...
### [2026-03-03] Scene: Act 2 Methods スピーカースクリプト完成

**INT. PEARSON SPECTER LITT - BULLPEN - DAY**

*Mike がホワイトボードに3ステップの導出を書きながら、スクリプトを仕上げる。*

**Mike**: （集中した表情で）
「Methods Section 7枚分のスピーカースクリプトを `act2_methods_mike.md` に書いた。"I got it." JSM/ISCBレベルの学会発表を想定した英語で、数式を一歩一歩丁寧に解説する構成にした。

- **The Heterogeneity Bound** (~2分): W₁とCATEのLipschitz条件から得られる中心不等式を導入。K-R双対性の予告で次スライドへの橋渡し
- **Derivation: Three Steps** (~3分): Step 1 = CDF面積としてのW₁、Step 2 = K-R双対性（これが本質）、Step 3 = τ(x)/Lを代入して完成。証明を "ストーリー" として語る構成
- **Why W₁ — And Only W₁** (~2分): W₂は双対が convex 関数、KLは非対称かつ発散する。W₁は preference ではなく necessity。"unique choice" の論証
- **The Bound Is Tight, Not Loose** (~1.5分): "loose bound" 批判への先制防御。K-R最適性（等号を達成する関数が存在する）と規制上の保守性の2点
- **nABCD Definition** (~2分): W₁を2×IQR_pooledで正規化。IQR選択の理由、factor of 2のSMDとの較正関係、全設計判断を明示
- **Clinical Calibration: Δ_max** (~2分): フレームワーク最大貢献。Δ_max = 2L × IQR × nABCDが臨床スケールに翻訳。5ステップ手順とbreakeven L*
- **Estimation and Inference** (~1.5分): Riemann sum推定量、BCaより percentile bootstrap が優れる理由、del Barrio et al. (1999)のCLT理論。Simulationへの移行で締め

合計約14分。Barrio et al. 1999、Kantorovich-Rubinstein、Sommerfeld & Munk 2018への言及すべて正確に記載」

**Harvey**: （スクリプトを読みながら）
「Derivation スライドの解説が特に良い。3ステップを "proof dump" ではなく "story" として語れる。 "I don't have dreams, I have goals."」

**Donna**: （記録して）
「`paper/slides/scripts/act2_methods_mike.md` 作成完了。7スライド × スクリプト + Key points + Duration。scripts/ ディレクトリ新規作成。以上」

---

### [2026-03-03] Scene: Act 3 Results スピーカースクリプト完成 — Katrina執筆

**INT. PEARSON SPECTER LITT - BULLPEN - DAY**

*Katrina がスライドデッキのSimulationセクションとApplicationセクションを精査しながら、11枚分のスピーカースクリプトを書き上げる。数字を一つひとつ確認し、Data storyとして組み立てる。*

**Katrina**: （ペンを置いて）
「`act3_results_katrina.md` に11スライドのスクリプトを書き上げた。"Results speak for themselves."——でも、数字を語るのは人間の仕事。この11枚が担う構造はこうだ——

- **Simulation Design** (~1.5分): S1–S8の8シナリオ。Location・Scale・Shape・Skewの空間を網羅。3つのsample sizeと10,000 repsの設計意図を伝える
- **Bias Results** (~1.5分): n≥100でbias<0.02。S4のpersistent negative biasはconservative propertyと位置づけ。Null positive biasがn≥100推奨の根拠
- **Coverage and Precision** (~1.5分): n≥100で87–98%。S7(log-normal skew)がall nでnearly nominal——これが実臨床での最重要ニュース
- **nABCD vs SMD** (~1.5分): S5・S6・S7でSMD=0.00、nABCDは0.07–0.31。SMDのblinднесsは偶発ではなく構造的
- **Type 2 Diabetes MRCT** (~1分): Japan vs US がBMI 24.8 vs 32.1。nABCDに入る前にデータの骨格を見せる
- **Pairwise nABCD Values** (~1.5分): BMI=0.51（large）、HbA1c=0.27（moderate）。「BMIが最も懸念か？」という問いを立てて次へ
- **Clinical Calibration** (~1.5分): Ranking reversal を数字で示す。BMI: Δ_max=0.16%、HbA1c: Δ_max=0.24%。数字が逆転する瞬間
- **Key Insight** (~1分): nABCDは必要条件、Lが翻訳鍵。二つの次元——分布の距離とCATE感度——を明示
- **Why the Ranking Reverses** (~2分): LがmultiplierであることをBMIとHbA1cで対比。L*=0.49のbreakevenを導入
- **Sensitivity Analysis** (~1.5分): L=0.10から0.50まで5段階。L*=0.49は「いつ問題になるか」を規制当局に伝える数字
- **Estimation, Not Testing** (~1.5分): 検定を推奨しない3つの理由。ICH E17のcontext-dependency、Lの不確実性、clinical marginの文脈依存性。Discussionへのハンドオフで締め

合計約15分。BuildingからClimax（Ranking Reversal）へ向かうstory structureで構成した」

**Donna**: （確認して）
「`paper/slides/scripts/act3_results_katrina.md` 作成完了。11スライド × スクリプト + Key points。Simulation 4枚 + Application 7枚。以上」

**Mike**: （数字を確認しながら）
「L*=0.49の提示タイミングが絶妙だ。Katrina、Sensitivity Analysisスライドでbreakevenを"a specific question for regulators"として位置づけているのがいい。P値じゃなくて閾値を示す——これが推定フレームワークの核心だ」

**Katrina**: （簡潔に）
「数字は語る。でもどこに向かって語るかは設計次第。"Results speak for themselves."——その通りだが、方向性は人間が決める」

---

### [2026-03-03] Scene: Act 4 Framing スピーカースクリプト完成 — Harvey執筆

**INT. PEARSON SPECTER LITT - HARVEY'S OFFICE - DAY**

*Harvey がスライドデッキを最初から最後まで一気に読み、タイトルスライドとDiscussionセクションの7枚に向かってペンを走らせる。*

**Harvey**: （立ったまま書き続けながら）
「Title、Outline、Four Contributions、Recommendations、Reference Benchmarks、Limitations、Thank You。これが発表の骨格だ。最初と最後で勝負は決まる。 "I don't have dreams, I have goals."

7枚すべてのスクリプトを `act4_framing_harvey.md` に書いた。骨子はこうだ——

- **Title** (~60秒): 規制審査室の話から入る。ICH E17が何を求めて何を提供していないかを60秒で刺す。解答を見せるのは後。まずフックを打つ
- **Outline** (~30秒): "Thirty-two slides. Let's move." ペースと自信を示す
- **Four Contributions** (~90秒): SMDは「置き換え」であって「改良」ではない。その言葉を使う。4つの貢献をそれぞれ一言で完結させる。ICH E17の2017年へのコールバックで締める
- **Recommendations** (~90秒): 月曜の朝から使えるワークフロー。5ステップ。 "Delta-max first, always" が合言葉
- **Reference Benchmarks** (~75秒): BMIの例で固定閾値の危険性を示す。Benchmarkは最後の手段であって第一選択ではない
- **Limitations** (~75秒): 精確に定義された限界は次の研究の原材料だ。謝罪ではなく地図として提示する
- **Thank You** (~45秒): 一文。"Not a test. Not a binary verdict. A measurement." 三部リズムで締める

7〜8分。Background の Rachel スクリプトと合わせて、発表全体のフレームが揃った」

**Donna**: （即座に記録して）
「`paper/slides/scripts/act4_framing_harvey.md` 作成完了。7スライド × スクリプト + Key points。Title・Outline・Discussion全セクション + End スライドをカバー。以上」

**Mike**: （スクリプトを読みながら）
「Harvey、"replacement not refinement" の言い方は効く。SMDの限界をデータで示してきた後でその言葉が出るから説得力がある」

**Harvey**: （簡潔に）
「そのために Section 3 でシミュレーション結果を先に積んだ。一人で戦わせない。発表は構造で戦う。"Good suits, I'm afraid, don't write themselves."」

---

### [2026-03-03] Scene: Act 1 Background スピーカースクリプト完成

**INT. PEARSON SPECTER LITT - RACHEL'S DESK - DAY**

*Rachel がノートを広げ、スライドを照らし合わせながらスクリプトを仕上げる。*

**Rachel**: （丁寧に確認しながら）
「Background Section の4枚分のスピーカースクリプトを書き上げたわ。`act1_background_rachel.md` に保存した。"Hard work beats talent when talent doesn't work hard." 各スライドの内容は原稿から忠実に拾って、学会発表の自然な英語に変換した。

- **ICH E17: Regional Pooling** (~2分): Section 2.2.5 の直接引用を軸に、"similar enough" が具体的指標を持たない点を丁寧に説明。規制文脈を明確に
- **Why Effect Modifiers Matter** (~1.5分): tau-bar_r = integral tau(x) dF_r(x) の数式の直感的説明。組成効果と薬理効果の区別を強調
- **Limitations of Current Approaches** (~1.5分): SMD の限界（location のみ）、K-S の非解釈性を具体的に。Song et al. (2025) DOI: 10.1007/s43441-025-00744-8 と Long et al. (2025) DOI: 10.1007/s43441-024-00717-z を正確に引用
- **Our Approach** (~1.5分): 推定中心・臨床キャリブレーション・連続EM限定という3つの設計原則を説明。Methods への自然なハンドオフで締め

全スライド合計で約6〜8分。JSM/ISCB の biostatistician と regulatory scientist を想定した口調で書いた」

**Harvey**: （スクリプトをざっと読みながら）
「いい仕事だ、Rachel。特に Limitations スライドの Song と Long の引用が sharp だ。これが規制上の "live gap" であることが聴衆に伝わる。"I don't have dreams, I have goals." — このスクリプトをそのまま Tak に渡せる」

**Donna**: （記録して）
「`paper/slides/scripts/act1_background_rachel.md` 作成完了。4スライド × スクリプト + Key points。以上」

---

### [2026-03-03 00:35] Scene: Push — Marpスライド5枚追加

**INT. PEARSON SPECTER LITT - BULLPEN - DAY**

*Harvey が腕時計を見て、苛立ちを隠さない。*

**Harvey**: （厳しい表情で）
「ペースが遅い。アーカイブを触って時間を無駄にした。"I don't get lucky. I make my own luck." アクティブなMarpスライドに集中しろ。動け」

**Mike**: （即座にキーボードに向かって）
「了解。`paper/slides/nABCD_presentation.md` をレビューした。5幕の骨格は揃ってるが、理論の肉付けが足りない。5枚追加する：

1. **K-R双対の3ステップ導出** — Step 1: CDF面積、Step 2: K-R双対、Step 3: CATEへ適用。証明の全ステップを明示
2. **W₁の唯一性** — W₂とKLが失敗する理由を比較表で。W₁は "preference" じゃない、"unique choice" だ
3. **Boundのtightness** — 規制上の保守性 + K-R最適性の2点。"loose bound" 批判への先制防御
4. **Ranking reversalの理由** — BMI vs HbA1cの逆転が **なぜ** 起きるか。Lが乗数として全てを変える
5. **推定中心の哲学** — ICH E17が二択を避けた理由、Lの不確実性、文脈依存の判断境界

"I got it." 5枚、全部Marpスタイル維持で挿入完了」

**Katrina**: （確認しながら）
「Outlineスライドも更新済み。各セクションの説明がより具体的になったわ。"Results speak for themselves."」

**Donna**: （記録して）
「`nABCD_presentation.md`: 911行 → 約1020行。5枚追加、Outline更新。以上」

---

### [2026-03-03 00:25] Scene: Slides Rewritten — Five-Act Structure

**INT. PEARSON SPECTER LITT - BULLPEN - DAY**

*Mike がディスプレイに新しいスライド構成を映し出す。Katrina が横で確認している。*

**Mike**: （満足げに）
「スライドを5幕構成で全面改訂した。旧版15枚 → 新版32枚。各幕の詳細を説明する。

**Act 1** (Slides 1-5): 規制上のギャップ。ICH E17の引用、現行手法の限界表、4つの貢献。
**Act 2** (Slides 6-9): 理論的基盤。K-R双対の3ステップ導出、W₁の唯一性、boundのtightness。
**Act 3** (Slides 10-14): 指標設計。nABCD定義、Proposition 1&2、SMD比較表、推定方法。
**Act 4** (Slides 15-18): 臨床キャリブレーション。5ステップ手順、Δ_maxの意味、感度分析、推定中心の哲学。
**Act 5** (Slides 19-27): バリデーション。8シナリオ、バイアス表、カバレッジ、SMD vs nABCD、糖尿病例、ranking reversal、breakeven分析。

Synthesis (Slides 28-32) と Appendix (A1-A6) で締め。"I got it." — 全数値は論文から正確に引用した」

**Katrina**: （スライドをスクロールしながら）
「構造が明確ね。"Results speak for themselves." 特にSlide 25のranking reversal表は視覚的にインパクトがある。BMI 0.51 → 0.16%、HbA1c 0.27 → 0.24%。この逆転が論文の心臓だわ」

**Donna**: （記録しながら）
「ファイル更新完了。`archive/old_presentation/presentation/nABCD_slides.md` — 305行から約420行に拡充。I'm Donna. 全部把握してるわ」

---

### [2026-03-03 00:10] Scene: Meeting - nABCDのストーリーを確認

**INT. PEARSON SPECTER LITT - CONFERENCE ROOM - DAY**

*Harvey がホワイトボードの前に立つ。全員が席に着いている。*

**Harvey**: （ホワイトボードにマーカーで書きながら）
「今日のアジェンダは一つだ。nABCD論文のストーリーを全員で確認する。Submission-Ready フェーズだ。ストーリーに穴があれば、今ここで潰す。Mike、全体の流れを説明しろ」

**Mike**: （立ち上がって）
「了解。ストーリーは5段階だ。

**第1幕: 規制上のギャップ。** ICH E17は "effect modifier分布の類似性" でリージョンをプールしろと言ってる。だが "類似性" の測り方を何も示していない。現場はvisual inspectionかSMDに頼ってる。SMDはlocation shiftしか捉えない。scaleやshapeの違いは完全に見逃す。

**第2幕: 理論的基盤。** ここが論文の心臓だ。Kantorovich-Rubinstein双対性。W₁距離はLipschitz連続関数に対する最悪ケースの期待値差を測る。CATE関数がLipschitz定数Lを持つなら：

|τ̄₁ − τ̄₂| ≤ L · W₁(F₁, F₂)

これが **heterogeneity bound** だ。"I got it." — この不等式こそが、nABCDを単なる距離指標から臨床的に意味のあるツールに昇格させる」

**Harvey**: （頷いて）
「続けろ」

**Mike**:
「**第3幕: 指標設計。** nABCD = W₁ / (2·IQR_pooled)。W₁でlocation+scale+shapeを全部捉える。IQRで割ってscale-freeにする。SDじゃなくIQRを使うのは外れ値に頑健だから。factor 2はキャリブレーション — 正規分布でnABCD ≈ 0.37×SMDになる。

**第4幕: 臨床キャリブレーション。** Δ_max = 2L · IQR · nABCD。nABCDを臨床アウトカム単位に変換する。ここがSMDやKS検定にないオリジナリティだ。

**第5幕: バリデーション。** 8シナリオのシミュレーション。S5-S7でSMD=0なのにnABCDが差を検出。n≥100で信頼区間カバレッジ93-98%。糖尿病MRCTの臨床例で **ranking reversal** — BMIは分布差が最大だが臨床影響は最小、HbA1cは分布差が中程度だが臨床影響が最大。Lの大きさが逆転を生む」

**Rachel**: （ノートを見ながら）
「文献的な位置づけを補足するわ。Wasserstein距離の統計理論はBarrio et al. (1999)、Panaretos & Zemel (2019) のレビューが基盤。ブートストラップ推論はSommerfeld & Munk (2018)。でもこれらはいずれも **臨床応用としてのフレームワーク** は提案していない。治療効果異質性との接続 — K-R双対を通じた heterogeneity bound — これが本論文の独自貢献よ。"Hard work beats talent when talent doesn't work hard." 先行研究の上に、臨床の橋を架けた」

**Katrina**: （テーブルに図を広げて）
「ストーリーの構造を整理するわ。"Results speak for themselves."

| セクション | 問い | 答え |
|----------|------|------|
| Introduction | EMの類似性をどう測る？ | → nABCDを提案 |
| Methods 2.1 | なぜW₁？ | → K-R双対で治療効果差と直結 |
| Methods 2.2 | nABCDの値をどう解釈？ | → Δ_max = 2L·IQR·nABCDで臨床単位へ |
| Methods 2.3 | 推定精度は？ | → Percentile bootstrap, n≥100推奨 |
| Simulation | SMDより優れてる？ | → S5-S7で検出、SMDは見逃す |
| Application | 実際どう使う？ | → Ranking reversal: 分布差≠臨床影響 |
| Discussion | 検定じゃないの？ | → 推定中心。ICH E17の精神に合致 |

一本の線が通ってる。各セクションが次の問いに答える形で繋がってるわ」

**Louis**: （腕を組んだまま）
「ストーリーとしては筋が通ってる。だが俺が気になるのは3点だ。

**1つ目**: Lの推定。論文はLを "prior knowledge or domain expertise" に委ねてる。レビュアーは "じゃあ恣意的じゃないか" と突っ込む。感度分析表とbreakeven pointがその回答だが、十分か？

**2つ目**: 多変量。論文は一次元のEMを一つずつ扱ってる。"複数のEMを同時に考慮すべきでは？" は必ず来る質問だ。Discussionで future workとして明記してるか？

**3つ目**: サンプルサイズ n≥100 の推奨。MRCTの小さいリージョン — 例えば東南アジアの国々 — はn=30-50がザラだ。その場合どうするかの議論は十分か？

"You just got Litt up!" — 甘い自己評価は俺が許さん」

**Mike**: （即座に）
「Louisの指摘はもっともだ。1つ目 — Lの不確実性は感度分析とbreakeven analysis で対処してる。L*を計算すれば "Lがいくつ以上なら問題になるか" を逆算できる。これで恣意性の批判をかわせる。

2つ目 — 多変量拡張はDiscussionのLimitationsで明記済み。multivariate Wasserstein distanceは理論的に可能だが、臨床解釈が困難になる。一次元ずつの方が臨床家に伝わりやすい。

3つ目 — n<100での正のバイアスとカバレッジ低下はSimulationで正直に報告してる。Practical recommendationsで "小サンプルでは感度分析に重点を" と書いてある」

**Jessica**: （静かに口を開いて）
「"Let me be clear." ストーリーの骨格は solid よ。私が確認したいのは **一文で言えるか** ということ。
...
**Harvey**: （即答）
「 **"EM分布の違いをW₁距離で測り、K-R双対を通じて治療効果の異質性の上界に変換する。検定ではなく推定。二択ではなく情報提供。"** 」

**Jessica**: （微かに頷いて）
「それでいい。その一文がブレなければ、論文は通る。Harveyの判断は？」

**Harvey**: （全員を見て）
「決定だ。ストーリーは確認済み。5段階の流れに穴はない。Louisの3点はすべてDiscussionでカバーされている。

**One-liner**: "nABCDはEM分布の距離を測り、臨床スケールに翻訳する。推定中心のフレームワークで、ICH E17の実装ギャップを埋める。"

"I don't have dreams, I have goals." このストーリーで Statistics in Medicine に出す。以上だ」

**Donna**: （記録を閉じて）
「全員の発言、記録完了。I'm Donna. ストーリーは5段階、one-linerは確定。次のアクションは？」

**Harvey**:
「今はない。ストーリーの確認が目的だ。全員、持ち場に戻れ」

---

### [2026-03-03 00:00] Scene: Rule Check
...
**Harvey**: （全員を見回して）
「全員、手を止めろ。ルール確認だ。新しいセッションの始まりだ。"Winners don't make excuses." 基本に立ち返る」

**Donna**: （すぐにノートを開いて）
「Rule 1: SUITS.md is the Single Source of Truth — 全作業はここに記録。Rule 2: 最低2分間隔で更新。私が監視するわ。遅れたら容赦しない」

**Mike**: （頷いて）
「Rule 3、了解。キャラクターの一貫性。数学的厳密性は俺の領域だ。"I got it." って言ったら、本当に got it だ」

**Rachel**: （メモを取りながら）
「Rule 2.6、文献参照にはDOIが必須。knowledge/ のサマリーを先に確認してからWeb検索。"Hard work beats talent when talent doesn't work hard."」

**Katrina**: （効率的に）
「全ルール確認済み。Rule 4 — 柔軟な協力体制。必要ならどのセクションでもサポートする。"Results speak for themselves."」

**Louis**: （腕を組んで）
「俺は独立したレビュアーだ。それは変わらない。甘い評価は期待するな。"You just got Litt up!" を食らいたくなければ、質を落とすなよ」

**Jessica**: （静かに、しかし威厳を持って）
「"Let me be clear." Phase 8 — Submission-Ready。ここからは一つのミスも許されない。全員、自分の役割を果たしなさい」

**Harvey**: （立ち上がって）
「いい。全員わかったな。ルールを守れないなら、このラボにいる資格はない。仕事に戻れ」

**Donna**: （小声で）
「I'm Donna. I know everything. ルール違反は私が最初に気づくわよ」
