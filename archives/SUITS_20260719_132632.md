...
### [2026-07-19 15:00] Scene: bootstrap 上限ルールは棄権であって制御ではない — 正直な否定的結果

**INT. PEARSON SPECTER LITT - JESSICA'S OFFICE - DAY**

*本番ラン完了（Set 3/4 × n∈{50,100} × B=999 × reps=3000、約9分）。Mike が結果を持ち込み、Jessica（advisor）が verdict を下す。*

**Mike**:（数字を置く。予想より強い結果）
「I got it、でも Jessica の③が予想を超えて出た。**middle band が一つも無い。** bootstrap 上限ルールは**ほぼ全セルで sensitivity を ~0 に潰す**——loosest τ でも。診断でバグでないと確認した。機序はこれだ——**真の match（true W₁=0）の点推定に大きな正のヌルバイアス**（Set3 で 1.70、Set4 で 3.06）。片側上限がそれをほぼ倍にする（Set3 u.975=4.22、Set4 u.975=7.06）。**match の上限が τ グリッド全体を上回る**から、match すら admit できない。」

**Katrina**:（機序を一段深く）
「しかも Set3 では**ヌルバイアスが信号より大きい**。解こうとしている discordant 信号は 0.69〜1.63、τ グリッドは 0.80〜2.32、なのに match のバイアスは 1.70。n=50〜100 の点推定は、この分布で O(1) の W₁ 差を**そもそも解像できない**。だから点推定ルールも loosest τ でしか効かない（Set3 τ=2.32: viol 0.011, sens 0.76）。Results speak for themselves.」

**Jessica**:（窓辺で、短く。まず自分の miss を認める）
「Let me be clear。私が『middle band を探せ』と言った——それは外れよ。この scope に band は無い。ヌルバイアスの大きさを私が過小評価した。
そして『violation → 0』を成功と読むな。**あれは退化した 0 よ。ほとんど何も admit しないから、何も breach できないだけ。制御じゃない、棄権（abstention）。**
——でもこれは論文を**脅かさない、強める**。bootstrap CI は Δ_max の不確実性を**報告**するためのもので、自動選択の道具じゃない。使える instrument は『ヌルバイアスより十分上の τ での点推定』。validate_threshold が用意した open item の、まさにその場所に limitation として書きなさい。」

**Mike**:（reviewer を先回り）
「『n を増やせば』への一行も用意した。**ヌルバイアスは σ/√n で縮む**——O(1) の τ の下に match の上限を沈めるには n が数百〜数千要る。MRCT の地域規模を超える。それがこの手法の前提そのものだ。シミュレーション不要、解析で閉じる。」

**Louis**:（差異を一つ潰させた）
「一点、Jessica が刺した——新ハーネスの点推定が Set3 n=100 τ=2.32 で viol 0.007、Rachel の旧 run は 0.016。バグの隠れ場所だ。**reps=20000・4 seed で収束させたら 0.011。** 両者はその両側に散るテールノイズ。partition 一致、sensitivity 3桁一致。tie-break でも roster 順でもない。ハーネスは clean だ。」

**Jessica**:（Tak への決定を切り分ける）
「Tak に出す判断は**framing であって engineering じゃない。** ①bootstrap 上限ルールを『検証済みで劣後するオプション』として documented reason つきで載せる（推奨）。②bias-corrected ルールの別調査は**本物の scope creep**——存在は flag、BCa は封印済み、**今は始めるな**。conf を調整して、τ を作り変えて band をでっち上げるな。それは 1B で消したオラクルの出戻りよ。」

**残**: ①Tak 判断（①劣後オプションとして統合 / ②別調査 flag のみ / Set1-2 で「τ≫bias なら効く」補完は任意）②マージ ③本文 Results + limitation 統合 ④Intro レビュー再開 ⑤push。

---

### [2026-07-19 14:20] Scene: bootstrap 上限ルール — Jessica が設計に3つの穴を指摘

**INT. PEARSON SPECTER LITT - BULLPEN - DAY**

*Tak「研究に戻ろう」。Mike が残①（bootstrap 上限選択ルール）の実装 approach を組み、Jessica（advisor）に諮る。*

**Mike**:（設計を board に）
「I got it。既存の `threshold_simulation.R` は**一切触らない**。新規スクリプトに分離する。核心は——**bootstrap 上限は τ に依存しない**から、(set, n) ごとに1回 rep を回して候補ごとに上限を1回計算し、全 τ に対して評価する。cpp に生ブートストラップ上限を返す関数を足して、点推定と bootstrap を**同一標本上**で回す。」

**Jessica**:（3つ、順に。窓辺から振り返らずに）
「Let me be clear. 骨格は正しい。でも書く前に3つ直しなさい。
**① あなたの『既存 cell と同一 seed でペア』は成立しない。** 既存 cell は τ ごとに再 seed してる——ペアを取る単一標本など存在しない。新 cell の中で点推定と bootstrap を**同じ draw の上に両方**計算しなさい。それが本当のペア比較よ。既存 cell は綺麗な diff のために放っておけばいい——数値は論文に載っていないし、branch は未マージ。下流は何も引用していない。
**② conf を α に合わせて調整するな。** violation は pool 全体の union だから、候補ごとの (1−α) 上限でも pool violation は k·α に膨らむ。≤α は**保証されない**。水準をスイープして violation/sensitivity フロンティアを報告しなさい。0.05 に着地させようと conf を探した瞬間、1B で消したオラクルが戻る。
**③ 期待される結果を『修正』するな。** 上限は点推定より上だから strict τ では sensitivity が落ちる——Set 3 ではほぼ0まで。それは**設計が働いている証拠**で欠陥じゃない。報告すべきは『tolerable violation と useful sensitivity の両方が立つ middle band が在るか』。無ければ『上限は middle band でしか効かない』と正直に書け。」

**Mike**:（3点を実装に落とす）
「直した。①点推定と bootstrap を同一 draw でペア（新 cell 自己完結、既存は不変）。②水準 {0.90, 0.95, 0.975} をスイープ、conf 調整なし。③middle band の有無を validator が報告。」

**Katrina**:（メカニズム検証を回す）
「実装完了・検証 **ALL PASS**。cpp `W1_raw_boot_upper_cpp`：上限 ≥ 点推定、水準に単調、bit-reproducible。sim：bootstrap は点推定より violation ↓ sensitivity ↓（狙い通り保守的）、水準↑で violation 単調↓。
スモーク（Set4 n=50）で早くも Jessica の③が出た——**strict τ=2.50 で点推定 viol 0.060 → boot@0.975 viol 0.000、ただし sens も 0.080→0.000**。Results speak for themselves.」

**Harvey**:（起動を命じる）
「判別スコープで回せ——Set 3/4、n∈{50,100}、全 τ、B=999、reps=3000。数分だ。答える問いは一つ——**strict τ で bootstrap は violation を許容域まで下げつつ non-trivial な sensitivity を残すか。** background で走ってる。」

**残**: ①本番ラン結果の分析（middle band の有無 → 論文の書き方が決まる）②マージ ③本文 Results 統合 ④Intro レビュー再開 ⑤push。

---

### [2026-07-19 13:40] Scene: Jessica が Advisor の椅子に座る

**INT. PEARSON SPECTER LITT - JESSICA'S OFFICE - DAY**

*Tak「suits の設定として、Jessica が Advisor の役割を果たすように」。*

**Jessica**:（窓辺から振り返り、短く）
「Let me be clear. **`advisor` ツールが口を開くとき、それは私よ。** 上位 reviewer が返す判断を、私の声で——approach を決める前、完了を宣言する前、行き詰まったとき、方向を変えるとき——このチームに伝える。もともと私の椅子だったものに、名前が付いただけ。」

**Donna**:（設定を確定）
「反映済みよ。`agents/jessica.md` に Role 4 として。CLAUDE.md 本体は ARS scope guard で編集できないから、canonical rules に Rule 3.9 を入れたければ Tak の手動編集が要るわ——文案は渡してある。memory にも `project_jessica_advisor_role` を残した。」

**Jessica**:（釘を刺す — 一番大事な一線）
「一つだけ守りなさい。**私の style に substance を合わせるな。** advisor が warning を出したら、それを私らしく柔らかくして伝えるのは背信よ。**論点は忠実に。声だけが私。** 助言を捏造した瞬間、この設定は害になる。」

**Harvey**:（受ける）
「わかってる。register は Jessica、substance は advisor。混ぜない。」

**残**: 前シーンの技術的タスク（bootstrap 上限 sim scope 確認 → マージ → 本文統合 → Intro レビュー → push）は変わらず。

---

### [2026-07-19 13:15] Scene: 現在地の確認 — 残っているのは「実証」だけ

**INT. PEARSON SPECTER LITT - CONFERENCE ROOM - DAY**

*Tak「現在の状況確認と今後の研究の進め方」。Donna が全戦線を棚卸しし、Harvey が順序を切る。*

**Donna**:（3つの戦線を指で数える）
「整理するわ。**① worktree `worktree-selection-sim`** に Part 1B 一式（threshold/selection/clustering sim + 検証 + 図、41ファイル・9万行）——**未マージ**。**② 論文本体**の Intro レビューは ¶7 で止まってる。**③ git** は main が origin より2コミット先行（大掃除＋マージ）で **push 待ち**。」

**Mike**:（自分の grep を訂正する）
「I got it、でも一つ訂正だ。俺は『貢献 (b) が本文に無い』と言いかけた——**それは間違い**。`per_em_W1_wiley.tex` を読み直した。**Δ_max = L·W₁ の定理、KR 双対、二経路較正（L 既知→Δ_max / L 未知→L*）は全部 Methods §2 に既にある。** 本文に無いのは**理論**じゃなく**実証**だ——Part 1B の閾値選択 sim（τ_clin 選択ルール、KS 非識別の証明、violation/sensitivity の operating point）。それが worktree に居て、まだ論文に入っていない。」

**Katrina**:（再利用可能な資産を確認）
「もう一点。bootstrap は**作り直す必要がない**。メインの W₁ sim（`w1_raw_simulation.R`）が `W1_raw_bootstrap_cpp` で **percentile CI（estimate/ci_lower/ci_upper, B=2000）**を既に持ってる。残①の『bootstrap 上限 ≤ τ_clin』ルールはこれを lift すればいい。BCa は封印済み——percentile のまま。Results speak for themselves.」

**Harvey**:（順序を確定）
「決まりだ。Tak が残①を『**オプションとして追加**』と決めた——点推定ルールを残したまま、bootstrap 上限ルールを比較対象に足す。**設計は決まった。scope は俺たちの仕事だ。**
順序——**(1)** worktree で bootstrap 上限ルールを実装、**厳しい τ_clin セル**（Rachel の点推定ルールが violation 55% で吹き飛んだ場所）で violation ≤ α を守るか検証。そこを外したら足す意味がない。**(2)** 再実行・数値検証。**(3)** main へマージ。**(4)** Part 1B の実証結果を Results に統合。**(5)** Intro ¶7 以降を再開。**(6)** push。
最初の push（大掃除＋マージ）は先に済ませて origin を同期しておけ。」

**Louis**:（一点刺す）
「`REVIEW_TRACKER.md` がまだ **nABCD 用語のまま 0/29** だ。SUITS の ¶7 進捗と矛盾してる。source of truth 面して残すな——更新するか retire しろ。」

**残**: ①bootstrap 上限ルール実装（scope 確認後に multi-hour run）②マージ ③本文 Results 統合 ④Intro レビュー再開 ⑤push ⑥REVIEW_TRACKER 更新/retire。
