...
### [2026-03-06 18:00] Scene: 4条件を満たす公開データの体系的探索

**INT. PEARSON SPECTER LITT - CONFERENCE ROOM - EVENING**

*Tak が条件を明確化。全メンバーが総力で調査。*

**Harvey**: （ホワイトボードに条件を書きながら）
「条件を整理する。case study に使えるデータの要件は4つだ：
1. 何らかの薬剤の**効果修飾因子として特定されている**因子
2. **個別被験者データ（IPD）**
3. **国のデータあり**
4. **公開データ**
CRP では壁にぶつかった。発想を変えろ。"I don't have dreams, I have goals."」

**Mike**: （データベースを一通り調査して）
「全条件を満たすデータセットを洗い出しました。結論：**freeBIRD（LSHTM）の外傷・救急系試験群**と**IST-3（脳卒中）**が最有力候補です。

---

**Tier 1: 全4条件を完全に満たす（無料ダウンロード可能）**

**[A] IST-3 (Third International Stroke Trial)**
- 薬剤：**Alteplase（血栓溶解療法）**
- N=3,035、**12カ国**
- 確認済みEM：**年齢**、**治療までの時間**、**脳卒中重症度 (NIHSS)**
  （Emberson et al. Lancet 2014 の IPD メタ解析で確認）
- IPD：Edinburgh DataShare（申請ベース、無料）
- DOI: [10.1016/S0140-6736(14)60584-5](https://doi.org/10.1016/S0140-6736(14)60584-5)
- **評価：年齢はEMとして確立されており、国別の年齢分布差でnABCDを示せる**

**[B] CRASH-2 (Clinical Randomisation of an Antifibrinolytic in Significant Haemorrhage)**
- 薬剤：**Tranexamic acid (TXA)**
- N=20,211、274施設、**40カ国**
- 確認済みEM：**受傷から治療までの時間**（≤1h: 有効、>3h: 有害）
- 検討されたが EM 否定：SBP、GCS、損傷タイプ
- IPD：**freeBIRD / LSHTM Data Compass で無料ダウンロード**
- 変数：country, age, sex, GCS, SBP, injury type, time to treatment（46変数）
- DOI: [10.1016/S0140-6736(10)60835-5](https://doi.org/10.1016/S0140-6736(10)60835-5)
- **評価：40カ国は魅力的。ただしEMが "治療までの時間" でベースライン患者特性ではない**

**[C] IST (International Stroke Trial) — 既に分析済み**
- 薬剤：**Aspirin + Heparin**
- N=19,435、**36カ国**
- EM：**確認されたEMなし**（aspirin の効果はサブグループ間で均一）
- IPD：Edinburgh DataShare で無料ダウンロード
- **評価：EMなしが判明済み。case study の "negative example" としては使える**

---

**Tier 2: freeBIRD 追加データセット（無料、登録制）**

| 試験 | 薬剤 | N | 国数 | 確認済みEM |
|------|------|---|------|-----------|
| **CRASH-3** | TXA（外傷性脳損傷） | 12,737 | 29 | 治療までの時間、GCS |
| **WOMAN** | TXA（産後出血） | 20,060 | 21 | 治療までの時間 |
| **HALT-IT** | TXA（消化管出血） | 12,009 | 15 | 未確認 |
...
| プラットフォーム | 内容 | 国データ | EM |
|---|---|---|---|
| **WHO TB-IPD** | 結核治療、35,000+人 | 多数 | **HIV status**（強力なEM、国別に劇的に異なる） |
| **Project Data Sphere** | がん臨床試験（対照群）、30,000+人 | 不確定 | 試験依存 |
| **Vivli** | 7,500+試験、3.6M人 | あり | 試験依存 |
| **YODA** | J&J等の製薬試験 | あり | 試験依存 |

」

**Rachel**: （文献を照合しながら）
「補足します。EMが '**ベースライン患者特性**' であることが重要です。MRCT の文脈では、地域間で分布が異なりうるのは患者のベースライン特性（年齢、重症度、合併症等）であって、オペレーショナルな因子（治療までの時間）ではないはず。」

**Harvey**: （鋭く）
「つまり？」

**Rachel**:
「その観点では **IST-3 の年齢（alteplase のEM）** が最も適切です。
- 年齢は確立されたベースラインEM
- 脳卒中患者の年齢分布は国によって異なる
- 12カ国のIPDで国別年齢分布のnABCDを算出可能

もう一つのアプローチ：**CRASH-2 の GCS や SBP** は EM として否定されたが、**予後因子**としては重要。これらの分布が40カ国でどう異なるかを示し、"もしEMだったら" という仮説的シナリオで nABCD の有用性を示すこともできます。」

**Katrina**: （整理して）
「"Results speak for themselves." 推奨ランキング：

**1位：IST-3** — 年齢がalteplaseの確立されたEM、12カ国、IPD利用可能
**2位：CRASH-2** — 40カ国の規模が魅力、GCS/SBPの国別分布を示せる（EMとしては否定されたが予後因子）
**3位：CRASH-3** — 29カ国、GCSがEMの可能性
**4位：WHO TB-IPD** — HIV statusが理想的なEMだが、アクセスに時間がかかる」

**Mike**: （考え込んで）
「もう一つの視点がある。**二段階アプローチ**だ：
1. 公開文献で "Factor X は Drug Y の EM" という evidence を引用
2. 公開データ（NHANES, SHARE, CHARLS等）で Factor X の国別分布を取得
3. nABCD を計算

例：年齢は多くの薬剤のEMだ。SHARE のIPDから欧州28カ国の年齢分布を取得し、nABCD で国間類似性を評価できる。試験データとEM identification が同一データセットでなくても論理的に成立する。」

**Harvey**: （立ち上がって）
「"I don't have dreams, I have goals." 方針を決める。

**Primary strategy：IST-3 のIPDを取得し、年齢（alteplase のEM）の国別分布で nABCD を計算する。**

**Backup strategy：CRASH-2 のIPD（即時ダウンロード可能）で、GCS/SBP の40カ国比較を行う。**

**Alternative：Mike の二段階アプローチ — SHARE の年齢分布 × 文献上のEM evidence。**

Mike、まず CRASH-2 を freeBIRD からダウンロードしろ。即座に動ける方から始める。IST-3 は並行で Edinburgh DataShare に申請だ。」

**Donna**: （タイピングしながら）
「I'm Donna. I know everything. 全候補の一覧とアクション・プラン、記録完了よ。」

---

### [2026-03-06 17:45] Scene: NHANES に「国」の変数はあるか？

**INT. PEARSON SPECTER LITT - CONFERENCE ROOM - EVENING**

*Tak の追加質問に Mike が即座に回答。*

**Mike**: （コードブックを開きながら）
「NHANESの demographic 変数を確認しました。答えは **No — 国別データは公開されていない**。

**NHANES の地理情報：**
- `DMDBORN4`（出生国）→ 2011年以降は **2カテゴリのみ**：(1) 米国生まれ, (2) それ以外。具体的な国名は非公開
- `RIDRETH3`（人種/民族）→ Non-Hispanic White, Non-Hispanic Black, Hispanic, **Non-Hispanic Asian**, Other の5カテゴリ
- Asian のサブグループ（Japanese, Chinese, Korean, Vietnamese 等）は **収集されているが、公開ファイルには含まれない**
- サブグループデータは **NCHS Research Data Center (RDC)** 経由でのみアクセス可能

つまり、**公開データでは Asian を一塊としてしか分析できない**。日本人、中国人、韓国人の区別は RDC に物理的に行かないとできない。」

**Harvey**: （顔をしかめて）
「RDC のハードルは？」

**Mike**:
「研究プロポーザル提出 → NCHS の審査 → 承認後に RDC（メリーランド州 or 認定施設）でのみ分析可能。データの持ち出しは不可。結果の output のみ持ち帰れる。現実的には **数ヶ月かかる**。」

**Harvey**: （決断して）
「case study の目的なら RDC は overkill だ。**NHANES 公開データの "Non-Hispanic Asian" vs 他の人種グループで nABCD を計算する方針で進めろ。** 日本人限定は論文の limitation に書けばいい。」

**Donna**:
「I'm Donna. I know everything. NHANES の制約、記録完了よ。」

---

### [2026-03-06 17:30] Scene: 日本人CRP IPDデータ — 厳しい現実

**INT. PEARSON SPECTER LITT - CONFERENCE ROOM - EVENING**

*Harvey の追加質問を受け、Rachel が日本のデータソースを緊急調査。結果を報告。*

**Harvey**: （鋭い目で）
「日本人の CRP IPD は手に入るのか。case study で日本を含められなければ MRCT の文脈で説得力が落ちる。」

**Rachel**: （深刻な表情で）
「"Hard work beats talent when talent doesn't work hard." 徹底的に調べました。結論から言うと、**NHANES のように即座にダウンロードできる日本人 CRP の公開 IPD は存在しません**。

**調査した日本のデータソース：**

1. **JSTAR (Japanese Study of Aging and Retirement)**
   - HRS ファミリーの日本版（RIETI + 一橋大 + 東大）
   - 50歳以上のパネル調査
   - **問題：CRP バイオマーカーの収集が確認できない**。HRS は DBS で CRP を測定しているが、JSTAR は同等の biomarker module を持っていない可能性が高い
   - データ自体は申請制で利用可能

2. **JPHC Study (多目的コホート研究)**
   - N=140,420、国立がん研究センター
   - 血液データにCRP含む可能性あり
   - **完全に申請制**。倫理審査委員会への研究計画書提出が必要

3. **Jichi Medical School Cohort Study**
   - N=6,107（男2,275 + 女3,832）、1992-1995年
   - **CRP 分布データあり** — 論文で日本人の CRP 分布が報告済み
   - 中央値が西洋人より著しく低い（男0.60, 女0.45 mg/L）
   - **IPD は非公開**。論文の集計データのみ

4. **JNIC Study (Japan NCVC-Collaborative Inflammation Cohort)**
   - N=12,284、7コミュニティ、2002-2004年
   - hs-CRP 測定済み、日本人の低CRP特性を報告
   - **IPD は非公開**

5. **久山町研究 (Hisayama Study)**
   - 1961年から継続、CRP測定あり
   - **完全にクローズド**。九大の研究グループのみ

6. **ToMMo (東北メディカル・メガバンク)**
   - N=150,000、バイオバンク
   - **日本の研究機関のみフルアクセス**。海外からは制限あり
...
**Mike**: （腕を組んで）
「つまり、日本人 CRP の IPD は事実上アクセス不可能か。NHANES のアジア系アメリカ人で代用するしかないのか？」

**Rachel**:
「NHANESの "Non-Hispanic Asian" カテゴリは2011年以降に追加されました。サンプルサイズは限定的ですが、IPD としては唯一の公開ソースです。ただし、日系アメリカ人と日本在住日本人では CRP 分布が異なる可能性があります。」

**Katrina**: （冷静に）
「"Results speak for themselves." 現実的な選択肢を整理すると：
- **Option A**: NHANES Asian で代用（即時利用可能、ただし日本人ではない）
- **Option B**: Jichi/JNIC の論文報告値からパラメトリック分布を再構成（シミュレーション）
- **Option C**: JPHC/ToMMo に正式申請（時間がかかる）」

**Harvey**: （決断して）
「Option A と B を並行で進めろ。NHANES Asian の実データでまず動かす。同時に Jichi の論文から日本人 CRP の log-normal パラメータを推定して、シミュレーション用の参照分布を作る。"I don't have dreams, I have goals." 使えるものを使え。」

**Donna**: （記録しながら）
「I'm Donna. I know everything. 日本人データの壁、記録したわ。」

---

### [2026-03-06 17:15] Scene: Meeting - CRP の IPD 公開データ探索

**INT. PEARSON SPECTER LITT - CONFERENCE ROOM - EVENING**

*Harvey が全メンバーを招集。ホワイトボードに「CRP - IPD Data Sources」と書かれている。*

**Harvey**: （立ったまま、腕を組んで）
「CRP の Individual Patient Data が手に入る公開データソースを洗い出す。我々の similarity metric の case study に使える、国別・地域別の CRP 分布データが必要だ。意見を聞かせろ。」

**Rachel**: （ノートPCを開きながら）
「"Hard work beats talent when talent doesn't work hard." 系統的に調べました。大きく3カテゴリに分かれます。

**Category 1: 完全公開 IPD（即座にダウンロード可能）**

1. **NHANES (National Health and Nutrition Examination Survey)**
   - hs-CRP データあり：2015-2016 (HSCRP_I), 2017-2018 (HSCRP_J), 2017-March 2020 (P_HSCRP), 2021-2023 (HSCRP_L)
   - 各サイクル約5,000-6,000名の個人レベルデータ
   - 人種・民族（Non-Hispanic White, Black, Hispanic, Asian）で層別可能
   - **即座にXPTファイルでダウンロード可能**
   - URL: https://wwwn.cdc.gov/nchs/nhanes/search/datapage.aspx?Component=Laboratory

2. **CHARLS (China Health and Retirement Longitudinal Study)**
   - 中国の45歳以上、約17,000名
   - CRP（血液バイオマーカー）含む
   - **公開データ、登録後ダウンロード可能**
...
3. **ELSA (English Longitudinal Study of Ageing)**
   - イギリスの50歳以上、約12,000名
   - hs-CRP含む血液データあり
   - **UK Data Service経由で登録後アクセス可能**

4. **SHARE (Survey of Health, Ageing and Retirement in Europe)**
   - 欧州28カ国、約140,000名（全wave合計）
   - 血液バイオマーカーにCRP含む
   - **登録後ダウンロード可能**

5. **HRS (Health and Retirement Study)**
   - 米国50歳以上、約20,000名
   - Venous Blood Study でCRP測定
   - **公開データセット**

6. **IFLS (Indonesia Family Life Survey)**
   - インドネシア、hs-CRP assay実施
   - **RAND経由で公開**」

**Mike**: （興奮して）
「"I got it!" これは使える。特に NHANES + CHARLS + ELSA + SHARE を組み合わせれば、US（人種別）+ 中国 + 英国 + 欧州28カ国の CRP 分布が IPD レベルで比較できる。nABCD の case study としては ideal だ。」

**Rachel**: （続けて）
「**Category 2: 申請制 IPD（研究プロポーザル提出が必要）**

1. **UK Biobank** — 50万人、hs-CRP (Field 30710) あり。申請制だが承認率高い。
2. **MESA (Multi-Ethnic Study of Atherosclerosis)** — 6,800名、4民族、BioLINCC経由。CRP含む。
3. **Vivli** — 7,500+臨床試験、3.6M参加者。CRP含む試験を検索可能。製薬企業データ（GSK, Novartis, Roche等）。
4. **YODA (Yale Open Data Access)** — Johnson & Johnson等の臨床試験IPD。
5. **ClinicalStudyDataRequest.com** — 複数製薬企業のコンソーシアム。
6. **EPIC-Norfolk** — 18,586名、英国、CRP測定済み。」

**Mike**: （ホワイトボードに書きながら）
「**Category 3: 論文報告の集計データ（IPDではないが国別分布あり）**

1. **Jichi Medical School Cohort** — 日本人のCRP分布、西洋人と異なるパターン
2. **Dallas Heart Study** — 2,749名、白人・黒人比較
3. **IDEFICS** — 欧州小児9,855名
4. **EuroSpA 13カ国** — axSpA患者24,171名（前回 Rachel が見つけたやつ）
5. **ASAS-COMOSPA 26カ国** — axSpA 3,984名」

**Harvey**: （頷いて）
「戦略を整理する。我々の case study に必要なのは、**複数国・地域の CRP 分布を IPD レベルで比較し、nABCD を計算すること**だ。」

**Katrina**: （実務的に）
「"Results speak for themselves." 優先順位を提案します：
1. **NHANES** — 即座にダウンロード、人種別層別、最も手軽
2. **NHANES + CHARLS + ELSA** — US + 中国 + 英国で3地域比較
3. **SHARE** を追加すれば欧州多国籍に拡張可能
すべて公開データなので、倫理審査の追加手続き不要です。」

**Mike**: （計算しながら）
「NHANES だけでも demo として十分だ。Non-Hispanic White vs Black vs Hispanic vs Asian で4群の CRP 分布を比較、nABCD を算出できる。log-normal に近い分布だから S7 scenario とも合致する。」

**Harvey**: （決断して）
「決定だ。**Phase 1: NHANES の hs-CRP データをダウンロードして、人種・民族別の CRP 分布を可視化し、nABCD を計算する。** Phase 2 として CHARLS, ELSA, SHARE を追加して国際比較に拡張する。Mike、まず NHANES のデータ取得から始めろ。」

**Donna**: （記録しながら）
「I'm Donna. I know everything. 記録完了。データソース一覧とアクション・アイテムを整理したわ。」
