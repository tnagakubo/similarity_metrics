...
### [2026-02-23 10:45] Scene: Meeting — 理解を深めるための説明資料

**INT. PEARSON SPECTER LITT - CONFERENCE ROOM - DAY**

*Harvey、ホワイトボードの前に立つ。Mike、Rachel、Katrina、Donnaが着席。Jessicaがドアの近くに立って見守る。*

**Harvey**: （腕を組んで）
「論文は書いた。スライドも作った。だがそれだけで十分か？ "Winning isn't everything. It's the only thing." 読者やレビュアーにnABCDを本当に**理解**させるための説明資料を考える。意見を聞かせろ。」

**Mike**: （ホワイトボードに走り書きしながら）
「"I got it!" まず現状を整理する。今あるのは：
1. **論文本体** — `nABCD_wiley.tex`（20ページ、理論+シミュレーション+応用）
2. **スライド** — `nABCD_presentation.md`（Marp、27枚、学会発表用）
3. **Rコード** — Appendix Cに`compute_nABCD`関数

足りないのは — レビュアーや規制当局の人が**直感的に理解できる素材**だ。数式の羅列だけでは、彼らのmindsetに刺さらない。」

**Rachel**: （資料を広げて）
「"Hard work beats talent when talent doesn't work hard." 文献調査の経験から言うと、Statistics in Medicineの **Tutorial Papers** セクションは特に教育的素材を重視する。以下のタイプの補足資料が効果的：

1. **Graphical Abstract / Visual Summary（図解サマリー）**
   - 1枚の図でnABCDの全体像を伝える
   - Problem → Method → Key Insight のフローを視覚化
   - SiMは投稿時のgraphical abstract提出を推奨している

2. **Worked Example Document（実例解説書）**
   - 糖尿病MRCTのデータを使って、ステップバイステップで計算過程を見せる
   - 数値例つきで「ここにこの数字を入れるとこうなる」を丁寧に
   - Rコードと数式を並列表示

3. **概念図の拡充**
   - 現在のFig 2（nABCD定義図）は良いが、**clinical calibrationの概念図**がない
   - 「nABCD → L → Δ_max → 臨床判断」のパイプラインを1枚の図に」

**Katrina**: （結果指向で）
「"Results speak for themselves." 私からは具体的な提案を3つ：

**A. Supplementary Tutorial Document**
- Appendix D として論文に添付、またはWeb Appendixとして別ファイル
- 内容: 5-step clinical calibrationの完全な数値例
- BMIとHbA1cの**ranking reversal**（nABCDが大きいのに臨床影響が小さい）を段階的に解説
- これが本論文の**最大のselling point**だから、ここを手厚く

**B. Quick Reference Card（クイックリファレンス）**
- A4片面、PDF 1枚
- nABCDの定義、計算手順、ベンチマーク表、Rコード呼び出し方法
- 実務家がデスクに置いて使える「チートシート」

**C. Decision Flowchart（意思決定フローチャート）**
- ICH E17のpooling判断プロセスの中でnABCDをどう使うか
- Yes/No分岐で視覚化：EM選定 → nABCD計算 → ベンチマーク確認 → Clinical calibration → 感度分析 → pooling判断」

**Louis**: （批判的に）
「Wait. "You just got Litt up!" 説明資料を作るのは構わないが、**本質的な問題**を見落とすな。レビュアーが最も引っかかるのは以下だ：

1. **Lipschitz constant Lの推定問題** — 論文で最大の弱点。説明資料でこそ、Lをどう設定するかの具体的ガイダンスが必要
2. **SMDとの比較の公平性** — SMDは location metric として設計されているのに、scale/shapeで勝つのは当然。これを「unfair comparison」と言われない説明が要る
3. **サンプルサイズの実現可能性** — n≥100 per regionを推奨しているが、小規模MRCTでは無理。small-nでの限界を正直に説明する資料も必要

これらを**先手で**説明資料に入れておけ。レビュアーに指摘される前に。」

**Jessica**: （静かに、しかし決定的に）
「"Let me be clear." 全員の意見は理に適っている。だが優先順位をつけなさい。投稿前に全てを作る必要はない。

**今すぐ作るもの（投稿に含める）：**
1. **Clinical calibration概念図** — 既存Figure群に追加、または Figure 2を拡張
2. **Web Appendix: Worked Example** — 数値例のステップバイステップ解説

**投稿後・リバイス時に検討：**
3. Graphical Abstract（レビュー結果次第）
4. Quick Reference Card（採択後の実務展開用）
5. Decision Flowchart（同上）

Louisの懸念は正しい。Worked Exampleの中にLの設定に関するガイダンスと感度分析の詳細を含めなさい。」

**Harvey**: （決断して）
「Perfect. Jessicaの優先順位で行く。

**即座のアクション：**
- **Mike**: Clinical calibration概念図のドラフト — nABCD → L × IQR → Δ_max のパイプラインを可視化
- **Katrina**: Web Appendix (Worked Example) の骨格を作成 — 糖尿病データで5-step calibrationを数値付きで
- **Rachel**: Worked ExampleにLの推定に関する文献根拠を補強
- **Louis**: ドラフトができたら、レビュアー目線で穴を突け

これは**投稿品質を上げる**ための作業だ。Let's move.」

**Donna**: （記録しながら）
「"I'm Donna. I know everything." 記録完了。アクションアイテム4件、担当割り振り済み。次のSUITS.md更新は各タスクの進捗報告時に。」
