# SUITS Quotes Collection

Character quotes for `/motivate` command.
Source: USA Network "SUITS" (2011-2019)

---

## Harvey Specter

### On Winning & Success
- "I don't have dreams. I have goals."
- "Winners don't make excuses when the other side plays the game."
- "Anyone can do my job, but no one can be me."
- "It's going to happen because I'm going to make it happen."
- "Don't play the odds. I play the man."
- "Work until you no longer have to introduce yourself."
- "That's the difference between you and me. You wanna lose small, I wanna win big."

### On Challenges
- "I don't get lucky. I make my own luck."
- "The only time success comes before work is in the dictionary."
- "When you're backed against the wall, break the goddamn thing down."
- "What are your choices when someone puts a gun to your head? You take the gun, or you pull out a bigger one."

### On Loyalty
- "Loyalty is a two-way street. If I'm asking for it from you, you're getting it from me."
- "You want to change your life? Change the way you think."

### On Confidence
- "Anyone can do my job, but no one can be me."
- "It's going to happen because I'm going to make it happen."
- "Ever loved someone so much, you would do anything for them? Yeah, well make that someone yourself and do whatever the hell you want."

---

## Jessica Pearson

### On Leadership
- "I'm not about caring. I'm about winning."
- "My name is on the damn door."
- "I've worked too hard to let anyone stop me now."
- "I don't respond well to threats."
- "Let me be clear."

### On Wisdom
- "In this game, you either win or you learn."
- "The next time you want to take someone down, make sure they're not standing on a ladder above you."

---

## Mike Ross

### On Growth
- "The bigger the struggle, the bigger the success."
- "I'm not going to let my past dictate my future."
- "Sometimes the hardest thing and the right thing are the same."
- "I got it!" (breakthrough catchphrase)

### On Problem Solving
- "There's always a solution. You just have to find it."
- "When you go after Harvey, you go after me too."

---

## Donna Paulsen

### On Identity
- "I'm Donna. I know everything."
- "I'm not into you. I'm Donna."
- "Trust me. If you were ever lucky enough to have me, you wouldn't want to share."

### On Capability
- "It's what I do. I read, I analyze, I fix."
- "I make everything work. That's what I do."
- "I know people usually better than they know themselves."

---

## Louis Litt

### Catchphrase
- "You just got Litt up!"
- "You're about to get Litt up."

### On Perfectionism
- "You want me to find the problems? That's what I do."
- "I'm the best at what I do because I'm thorough."
- "Attention to detail is not a weakness. It's a strength."

### On Self
- "I may not be as flashy as Harvey, but I get results."
- "I'm Moby goddamn Dick, and you just swam in my waters."

### On Life & Mudding
- "There's nothing to be afraid of—it's just good clean mud."
- "Do you mud?"
- "I treat my body like a temple. Does that make me uncool? I don't think so."
- "Don't let the moment pass. Don't let the people you love walk by you without letting them know how you feel about them."

---

## Rachel Zane

### On Determination
- "I'm going to prove myself, no matter what it takes."
- "Hard work beats talent when talent doesn't work hard."
- "The truth is, we all have skeletons in our closets."
- "I've worked too hard to give up now."

---

## Katrina Bennett

### On Loyalty
- "Litt the hell up."
- "My allegiance is to Louis."

---

## Usage in /motivate

```
**Harvey**: "'Winners don't make excuses.'
Excuses are for losers. We're winners. Let's figure out the next move."
```

```
**Louis**: "'You just got Litt up!'
You passed my review. That's something to be proud of."
```

```
**Donna**: "'I'm Donna. I know everything.'
I see everything. I know how hard you've worked."
```
