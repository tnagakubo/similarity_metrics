...
| Monte Carlo repetitions | **Part 1: 10,000** per cell · **Part 2: 5,000** per cell. **Precision is stated per measure type, because the measures are not the same kind of quantity:** <br>• *Unit-free measures in [0,1]* (AUC, precision@k, false-pooling@k, ARI): absolute **MC SE ≤ 0.005** (Part 1 max 0.0050; Part 2 ARI max 0.0039). <br>• *Part 2 exact-recovery*: a proportion, so its SE at 5,000 reps peaks at √(0.25/5000) = **0.0071** near p = 0.5 (observed 0.0070) — a structural bound, not a deficiency. <br>• *Clinical harm*: E[max true W₁] is a **scale quantity in EM units** (values 0.3–10), so an absolute 0.005 bound would be a category error. Judged on **relative** precision: **max relative MC SE = 3.6 %** (absolute max 0.043). The headline it exists for — Set 4, KS 4.27 vs W₁ 2.00 — is **106 MC standard errors apart**, with non-overlapping CIs. |
