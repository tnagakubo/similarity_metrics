...
parse_args <- function(a = commandArgs(trailingOnly = TRUE)) {
  ...
  out <- list(reps = 5000L, n_grid = c(25L, 50L, 75L, 100L),
              base_seed = 20260712L, tag = "")
  for (x in a) {
...
main <- function() {
  ...
  logcon <- file(file.path(results_dir, sprintf("clustering_sim%s.log", OPTS$tag)), open = "wt")
  ...
  f_sum <- sprintf("clustering_sim_summary%s.csv", OPTS$tag)
  f_cfg <- sprintf("clustering_sim_config%s.csv",  OPTS$tag)
  write.csv(summary_df, file.path(results_dir, f_sum), row.names = FALSE)
  write.csv(do.call(rbind, cfg_rows), file.path(results_dir, f_cfg), row.names = FALSE)
