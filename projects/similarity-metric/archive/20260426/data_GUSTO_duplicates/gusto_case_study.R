...
#   moderate demographics). Effect modifiers: sysbp and age.
#
...
fig_dir <- file.path(dirname(dirname(out_dir)), "figures")
...
em_vars <- c("sysbp", "age")
all_vars <- c("age", "sysbp", "pulse", "height", "weight")
...
cat("Planned MRCT: Phase 3, 15 regions (Region 5 excluded from pooling exercise)\n\n")
...
# Anchor: Region 13
# Rationale: Region 13 has a large sample (N~3000+), moderate demographics
# that place it near the center of the global distribution, making it a
# clinically natural reference -- NOT chosen for small size.
anchor <- 13
...
cat("  Note: In MRCT planning, the anchor region need not have small N.\n")
cat("  The anchor represents the region from whose perspective pooling\n")
cat("  partners are evaluated -- typically the region with the best-\n")
cat("  characterized patient population or the sponsor's home region.\n\n")
...
cat("   - This shape variation is precisely the scenario where nABCD adds value\n\n")
cat("2. Age: Secondary EM (comparator)\n")
...
for (v in em_vars) {
  ...
  if (nrow(divergent) == 0) {
    ...
    # Show top 3 by absolute rank_diff
...
# =============================================================================
# SECTION 5: Bootstrap CI for All Anchor-Partner Pairs
# =============================================================================
cat("=============================================================================\n")
...
# For acute MI thrombolytic trials:
# - Overall treatment effect ~ 1%pt absolute mortality reduction (GUSTO reference)
# - Non-inferiority margin ~ 0.5%pt (hypothetical for Drug T)
# - Delta_clin = 1%pt (equals overall treatment effect)
...
cat("--- 6a. L* Sensitivity Analysis for sysbp (Case A analog) ---\n")
cat("When CATE sensitivity L is unknown, we compute L*: the L value that would\n")
cat("make the observed distributional difference clinically meaningful.\n\n")
...
cat("\n")
cat("Interpretation: L* represents the minimum CATE sensitivity required for\n")
cat("the observed sysbp distributional difference to produce a Delta_max\n")
cat("equal to the overall treatment effect. If the true L is plausibly\n")
cat("below L*, the distributional difference has low clinical risk.\n\n")

# --- 6b. Hypothetical L estimation for age (Case B analog) ---
cat("--- 6b. Clinical Calibration for Age (Case B analog) ---\n")
cat("For age, we hypothesize L based on known AMI epidemiology:\n")
cat("  - FTT Collaborative Group (1994) showed 30-day mortality increases\n")
cat("    approximately 0.3%pt per year of age for thrombolytic vs control\n")
cat("  - Conservative estimate: L_mean ~ 0.0003/year (per-year gradient)\n")
cat("  - Upper bound: L_max ~ 0.0005/year\n\n")
...
for (i in seq_len(nrow(res_age))) {
  ...
  dmax_mean <- compute_delta_max(nA, iqr_p, L_age_mean) * 100  # to %pt
...
cat("\n")
cat("Delta_max expressed in %pt. Dmax/TE = ratio to overall treatment effect.\n")
cat("Values < 1 suggest the distributional difference is unlikely to produce\n")
cat("heterogeneity exceeding the overall treatment effect.\n\n")

# --- 6c. Bootstrap CI for Delta_max (age, top divergent pairs) ---
cat("--- 6c. Bootstrap Delta_max CI for Age (top 5 most different) ---\n\n")

res_age_sorted <- res_age %>% arrange(desc(nABCD))
top5_age <- head(res_age_sorted$partner, 5)

x_anchor_age <- gusto[["age"]][gusto$regl == anchor]
x_anchor_age <- x_anchor_age[!is.na(x_anchor_age)]

cat(sprintf("%-8s  %8s  %10s  %10s  %10s  %10s  %10s\n",
            "Partner", "nABCD", "95%CI_low", "95%CI_up",
            "Dmax_mn", "Dmax_low", "Dmax_up"))
cat(strrep("-", 80), "\n")

for (r in top5_age) {
  x_partner <- gusto[["age"]][gusto$regl == r]
  x_partner <- x_partner[!is.na(x_partner)]
  iqr_p <- IQR(c(x_anchor_age, x_partner))

  ci <- nABCD_bootstrap_ci(x_anchor_age, x_partner, B = 2000)
  dmax_est <- compute_delta_max(ci$estimate, iqr_p, L_age_mean) * 100
  dmax_low <- compute_delta_max(ci$lower, iqr_p, L_age_mean) * 100
  dmax_up <- compute_delta_max(ci$upper, iqr_p, L_age_mean) * 100

  cat(sprintf("%-8d  %8.4f  %10.4f  %10.4f  %10.3f  %10.3f  %10.3f\n",
              r, ci$estimate, ci$lower, ci$upper,
              dmax_est, dmax_low, dmax_up))
}
cat("\nDelta_max values in %pt, using L_mean = 0.0003/year.\n\n")
...
# nABCD threshold from benchmarks: < 0.05 negligible, 0.05-0.15 small
# SMD conventional threshold: |SMD| < 0.10
...
for (v in em_vars) {
  ...
  smd_moderate <- res %>% filter(abs_SMD >= 0.10 & abs_SMD < 0.20)
...
# =============================================================================
# SECTION 8: Summary Table for Paper
# =============================================================================
cat("=============================================================================\n")
cat("SECTION 8: Summary Table (for LaTeX)\n")
cat("=============================================================================\n\n")

cat("Combined ranking table (sysbp + age, sorted by mean nABCD):\n\n")
...
combined <- anchor_results[["sysbp"]] %>%
  ...
  mutate(
    mean_nABCD = (sysbp_nABCD + age_nABCD) / 2,
    mean_SMD = (sysbp_SMD + age_SMD) / 2
  ) %>%
  arrange(mean_nABCD)

cat(sprintf("%-6s  %5s  %8s  %8s  %5s  %8s  %8s  %5s  %8s  %8s\n",
            "Reg", "N", "sysbp_nA", "sysbp_SM", "rkDf",
            "age_nA", "age_SM", "rkDf", "mean_nA", "mean_SM"))
cat(strrep("-", 90), "\n")

for (i in seq_len(nrow(combined))) {
  cat(sprintf("%-6d  %5d  %8.4f  %8.4f  %+5d  %8.4f  %8.4f  %+5d  %8.4f  %8.4f\n",
...
# --- Figure A: nABCD vs |SMD| scatter for both EMs ---
...
# Show anchor + Region 5 (extreme skew) + most similar + most different
res_sysbp <- anchor_results[["sysbp"]]
most_similar <- res_sysbp %>% arrange(nABCD) %>% head(1) %>% pull(partner)
most_different <- res_sysbp %>% arrange(desc(nABCD)) %>% head(1) %>% pull(partner)
spotlight_regions <- c(anchor, 5, most_similar, most_different)
spotlight_regions <- unique(spotlight_regions)
...
# --- Figure C: Rank divergence lollipop chart ---
rank_data <- bind_rows(
  anchor_results[["sysbp"]] %>%
...
p_rank <- ggplot(rank_data, aes(x = factor(partner), y = rank_diff,
  ...
  geom_hline(yintercept = c(-2, 2), linetype = "dashed", colour = "red",
             alpha = 0.5) +
  scale_fill_manual(values = c("Systolic BP" = "#D55E00", "Age" = "#0072B2")) +
  labs(
...
# --- Figure D: Bootstrap CI forest plot ---
...
cat("     demographic centrality, NOT for small sample size\n")
cat("   - Two effect modifiers: sysbp (shape-dominant) and age (location-dominant)\n\n")

cat("2. nABCD vs SMD DIVERGENCE (key finding for paper)\n")
# Print the key sysbp result
...
cat("   Lower rho for sysbp confirms greater shape heterogeneity.\n\n")

cat("4. CLINICAL CALIBRATION COMPARISON\n")
cat("   sysbp: L unknown -> L* sensitivity analysis shows that even modest\n")
cat("   CATE sensitivities could produce meaningful heterogeneity for\n")
cat("   high-nABCD pairs (parallels Case A scenario from IST-1).\n")
cat("   age: L estimated -> Delta_max generally small relative to TE,\n")
cat("   confirming age distributional differences pose low clinical risk\n")
cat("   (consistent with IST-3 age finding).\n\n")

cat("5. CONTRAST WITH IST CASES\n")
cat("   - IST-1/IST-3 (stroke): Effect modifiers show primarily location-dominated\n")
cat("     heterogeneity (age, NIHSS), with treatment delay as the shape exception.\n")
cat("   - GUSTO (MI): Systolic BP shows strong shape heterogeneity across regions\n")
cat("     (skewness range 0.03-1.32), providing a complementary case where\n")
cat("     nABCD's distributional sensitivity is essential for correct assessment.\n")
cat("   - The GUSTO case reinforces the paper's central message: nABCD captures\n")
cat("     distributional features invisible to SMD, and the clinical relevance\n")
cat("     depends on the specific EM and its CATE sensitivity.\n\n")
