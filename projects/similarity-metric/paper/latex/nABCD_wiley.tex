...
\abstract[Abstract]{%
The ICH E17 guideline recommends regional pooling in multi-regional clinical trials (MRCTs) based on similarity of effect modifier (EM) distributions, but provides no specific methodology for quantifying such similarity. Existing approaches focus on location differences (standardized mean difference) or lack interpretable scales (Kolmogorov--Smirnov statistic). We propose the normalized Area Between Cumulative Distributions (nABCD), defined as the Wasserstein-1 distance between two distributions divided by twice the pooled interquartile range. Unlike the standardized mean difference, nABCD captures differences in variance, shape, and skewness---precisely the distributional features that may drive treatment effect heterogeneity through non-linear effect modification. When prior evidence on the conditional average treatment effect (CATE) sensitivity is available, nABCD enables clinical calibration: the maximum potential regional treatment effect difference ($\Delta_{\max}$) can be expressed as a function of nABCD, the pooled IQR, and the CATE sensitivity parameter, with bootstrap confidence intervals quantifying estimation uncertainty. Simulation studies across eight scenarios at sample sizes typical in MRCT regional subgroups demonstrated satisfactory bias and coverage of the percentile bootstrap estimator. Application to two complementary historical datasets illustrates the framework's practical flexibility: the First International Stroke Trial (IST-1; $n = 19{,}435$, 31 countries) demonstrates Case A (EM unknown) using global-scale geographic heterogeneity and $L^*$ sensitivity analysis, while the Third International Stroke Trial (IST-3; $n = 3{,}035$, 8 countries) demonstrates Case B (EM identified) with clinical calibration. In IST-3, nABCD detected distributional differences invisible to the standardized mean difference, yielding markedly different clinical conclusions depending on CATE sensitivity: NIHSS (nABCD $= 0.240$) translated to substantial potential heterogeneity ($\Delta_{\max} = 5.02$ percentage points), whereas age (nABCD $= 0.285$), despite larger distributional magnitude, had limited impact ($\Delta_{\max} = 0.47$ percentage points). Treatment delay exemplified nABCD's distributional sensitivity: Norway--Portugal showed SMD $\approx 0$ yet nABCD $= 0.069$, a divergence from scale and shape differences alone. The nABCD framework fills a methodological gap in ICH E17 implementation by providing a principled distributional comparison measure with an optional clinical calibration pathway, supporting evidence-based pooling decisions across the spectrum from EM-unknown to EM-identified scenarios.}
...
The remainder of this paper is organized as follows. Section~\ref{sec:methods} presents the methodological framework. Section~\ref{sec:simulation} describes a comprehensive simulation study. Section~\ref{sec:application} demonstrates the framework using publicly available individual patient data from the IST-3 stroke trial, framed as a prospective planning exercise for a hypothetical thrombolytic MRCT. Section~\ref{sec:discussion} discusses implications, limitations, and future directions.
...
S2 & Location 0.2$\sigma$ & $N(50, 10^2)$ & $N(52, 10^2)$ & 0.074 \\
...
S7 (Skew only) & $0.311 \pm 0.047$ & $0.00 \pm 0.14$ & Only nABCD detects \\
\bottomrule
\end{tabular*}
\begin{tablenotes}
...
\begin{enumerate}[1.]
\item \emph{Bias}: Less than 0.02 for most non-null scenarios at $n \geq 100$, with S4 showing persistent bias of $-0.04$ due to boundary effects at large true values
\item \emph{Coverage}: 0.87--0.98 at $n \geq 100$ for most scenarios; new scenarios S7 (skew) and S8 (location + scale) achieved coverage between 0.93--0.95
...
We illustrate the nABCD framework through a hypothetical scenario grounded in publicly available clinical trial data. A sponsor is developing a novel thrombolytic agent (drug~A) for acute ischaemic stroke and is planning a Phase~3 MRCT. Based on the Emberson et al.\cite{emberson2014} IPD meta-analysis of nine alteplase trials ($n = 6{,}756$), NIHSS baseline stroke severity (treatment $\times$ EM interaction $p = 0.06$) and age ($p = 0.53$) are identified as candidate EMs for the thrombolytic class. Because drug~A belongs to the same pharmacological class as alteplase, similar EM profiles are assumed. To characterize country-level EM distributions for the planned MRCT regions, the sponsor uses publicly available IPD from the Third International Stroke Trial (IST-3)\cite{ist3_2012} as a historical reference. We emphasize that nABCD serves as one piece of evidence supporting pooling decisions, not a standalone decision criterion.
...
Both trials are now retrospectively analyzed as methodological illustrations of the nABCD framework. Neither was prospectively designed as an MRCT, yet together they exemplify the decision environment sponsors face during actual Phase~3 planning.
...
Age has weak evidence as an EM: Emberson et al.\cite{emberson2014} reported $p = 0.53$, and the IST-3 data confirm non-significance ($p = 0.614$). The estimated CATE sensitivity is small ($L_{\text{mean}} = 0.00065$/yr) with wide uncertainty.
...
\begin{enumerate}[1.]
\item \emph{Post-hoc analysis}: IST-3 was not designed as an MRCT; this application demonstrates methodological feasibility, not prospective use.
...
\item \emph{Temporal generalizability}: IST-3 enrolled patients during 2000--2011; demographic shifts in stroke populations may limit applicability to current trial planning.
\item \emph{Geographic coverage}: Countries in IST-3 may not match planned Phase~3 regions; the EM distributions serve only as historical reference.
\end{enumerate}
