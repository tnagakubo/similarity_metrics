...
\item For S4 (large effect), coverage decreased at larger sample sizes due to increased precision revealing the persistent negative bias. The modest coverage degradation at small nABCD values (near the null $F_1 = F_2$) is consistent with known theoretical properties of bootstrap inference for Wasserstein distances: at $F_1 = F_2$, the Hadamard derivative of $W_1$ becomes non-linear, which can affect bootstrap consistency.\cite{sommerfeld2018} This boundary behavior diminishes with increasing sample size and increasing distributional separation, as confirmed by the near-nominal coverage observed for S3--S8 at $n \geq 100$.
\end{tablenotes}
\end{table}
...
We illustrate the nABCD clinical calibration framework using a hypothetical MRCT in type 2 diabetes with three regions: Japan ($n = 150$), United States ($n = 200$), and European Union ($n = 180$). The primary endpoint was change in HbA1c (\%) at 24 weeks, with an overall treatment effect of $-0.8$\% and a pre-specified non-inferiority margin of $\Delta_{\text{clin}} = 0.4$\% HbA1c. This example directly addresses the ICH E17 guideline's emphasis on understanding regional differences in intrinsic and extrinsic ethnic factors:\cite{iche17} nABCD quantifies the distributional similarity of such factors across regions, providing the quantitative foundation that ICH E17 calls for but does not prescribe. Table~\ref{tab:baseline} presents baseline characteristics by region.
...
\item \emph{BMI}: $L_{\text{BMI}} \approx 0.02$\% HbA1c per kg/m$^2$. A regression analysis of DPP-4 inhibitor efficacy estimated a BMI coefficient of $-0.02$ per kg/m$^2$ ($p = 0.024$) for the relationship between BMI and HbA1c reduction.\cite{yagi2015}
...
This example demonstrates that the clinical meaning of nABCD depends on the EM's role as an effect modifier. A large nABCD for a weak EM (BMI, $L = 0.02$) may be less consequential than a moderate nABCD for a strong EM (HbA1c, $L = 0.30$). Because $L$ is rarely known with precision, we recommend that practitioners report nABCD results across a range of plausible Lipschitz constants $L$, as the clinical interpretation depends critically on this parameter. Point estimates of $\Delta_{\max}$ at a single $L$ should be accompanied by sensitivity analysis showing how conclusions change across the plausible range. Table~\ref{tab:sensitivity} illustrates this recommended practice for baseline HbA1c.
...
The choice of Wasserstein distance over information-theoretic divergences such as Kullback--Leibler (KL) divergence warrants discussion. While KL divergence is a natural measure of distributional difference in information theory, it has structural limitations for the present application. First, $D_{\text{KL}}(P \| Q) \neq D_{\text{KL}}(Q \| P)$, meaning that comparing Region A to Region B yields a different value than comparing B to A---an asymmetry inconsistent with the symmetric nature of regional pooling decisions in MRCTs. Second, KL divergence can diverge to infinity when the support of $Q$ does not cover the support of $P$, a common occurrence with empirical distributions that would require density estimation and bandwidth selection, introducing additional tuning parameters and complexity. Third, and most critically, KL divergence lacks the Lipschitz-bound property central to our framework: the Kantorovich--Rubinstein duality theorem ensures that Wasserstein distance bounds the difference in expectations of Lipschitz-continuous functions, enabling Proposition~2's heterogeneity bound. KL divergence has no analogous result linking it directly to treatment effect heterogeneity for Lipschitz-continuous CATE functions.\cite{gibbs2002} The Wasserstein distance is a true metric satisfying the triangle inequality and metrizing weak convergence,\cite{villani2008} making it the natural choice for our clinical calibration framework.
...
\item As with any covariate-based approach, nABCD assesses similarity only with respect to measured effect modifiers. Unmeasured or unknown effect modifiers may contribute to treatment effect heterogeneity beyond what the metric captures.
\end{enumerate}
...
An R package implementing the nABCD metric with bootstrap inference and the clinical calibration framework described herein is under development.
