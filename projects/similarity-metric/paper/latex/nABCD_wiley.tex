...
\abstract[Abstract]{%
The ICH E17 guideline recommends regional pooling in multi-regional clinical trials (MRCTs) based on similarity of effect modifier (EM) distributions, but provides no specific methodology for quantifying such similarity. Existing approaches focus on location differences (standardized mean difference) or lack interpretable scales (Kolmogorov--Smirnov statistic). We propose the normalized Area Between Cumulative Distributions (nABCD), defined as the Wasserstein-1 distance between two distributions divided by twice the pooled interquartile range. Through its connection to the treatment effect heterogeneity bound, nABCD enables clinical calibration: the maximum potential regional treatment effect difference ($\Delta_{\max}$) can be expressed as a function of nABCD, the pooled IQR, and the CATE sensitivity of the EM, with bootstrap confidence intervals quantifying estimation uncertainty. Simulation studies across eight scenarios---including location shifts, scale differences, skewed distributions, and combined patterns---at sample sizes typical in MRCT regional subgroups demonstrated satisfactory bias and coverage of the percentile bootstrap estimator. Unlike the standardized mean difference, nABCD captured variance, shape, and skewness differences that are invisible to location-only metrics. Application to a diabetes MRCT illustrated that clinical calibration provides context-dependent interpretation: a large distributional difference in a weak effect modifier (BMI: nABCD $= 0.51$, $\Delta_{\max} = 0.16$\%) carried less clinical weight than a moderate difference in a strong effect modifier (baseline HbA1c: nABCD $= 0.27$, $\Delta_{\max} = 0.24$\%), demonstrating that distributional magnitude and clinical impact are fundamentally different dimensions. The nABCD framework fills a methodological gap in ICH E17 implementation by translating distributional differences into potential treatment effect heterogeneity on the clinical scale, supporting evidence-based pooling decisions.}
...
This paper addresses the methodological gap by proposing the \emph{normalized Area Between Cumulative Distributions} (nABCD), a metric for comparing EM distributions across regions, together with a clinical calibration framework that translates distributional differences into potential treatment effect heterogeneity on the outcome scale. Our specific research question is:
...
Geometrically, this equals the total area between the two CDFs (Figure~\ref{fig:nabcd_definition}). Unlike the standardized mean difference, the Wasserstein distance responds to changes in variance, skewness, and other distributional features.\cite{panaretos2019} We use $W_1$ rather than higher-order Wasserstein distances because the Lipschitz bound in equation~(\ref{eq:heterogeneity_bound}) requires the $W_1$ distance specifically. By the Kantorovich--Rubinstein duality, $W_1(F_1, F_2) = \sup_{\|f\|_{\text{Lip}} \leq 1} |\int f \, dF_1 - \int f \, dF_2|$, which directly connects Lipschitz-continuous functions (including the CATE) to distributional distance.\cite{villani2009} The $W_2$ distance, while admitting closed-form expressions for Gaussian families, does not possess this dual characterization via Lipschitz functions and therefore cannot provide the heterogeneity bound that is central to our clinical calibration framework.
...
\textup{(Non-negativity).} For distributions with finite IQR, $\text{nABCD} \geq 0$, with equality if and only if $F_1 = F_2$.
\end{proposition}
...
When prior knowledge of $L$ is unavailable, the benchmarks in Table~\ref{tab:benchmarks} provide a convenience reference for initial assessment. These labels describe \emph{distributional magnitude} only and should not be conflated with clinical significance: as Section~\ref{sec:application} demonstrates, a ``large'' nABCD for a weak effect modifier may carry less clinical consequence than a ``moderate'' nABCD for a strong effect modifier, because CATE sensitivity $L$ determines how distributional distance translates into treatment effect heterogeneity.
...
\end{tabular*}
\begin{tablenotes}
\item These benchmarks describe distributional magnitude assuming moderate CATE sensitivity and do not indicate clinical significance. The same nABCD value may be negligible for one EM and consequential for another depending on CATE sensitivity $L$. Actual pooling decisions should use clinical calibration (equation~\ref{eq:delta_max}) whenever possible.
\end{tablenotes}
\end{table}
...
<<<<<<< HEAD
We designed eight scenarios spanning location, scale, shape, and combined differences (Table~\ref{tab:scenarios}). Each scenario uses $N(50, 10^2)$ as the reference distribution (Region~1), with Region~2 varying to create specific distributional differences motivated by realistic MRCT patterns. Scenarios S1--S4 examine location shifts of increasing magnitude, S5 examines a scale difference arising from heterogeneous inclusion criteria, S6 examines a shape difference from right-skewed lab values, S7 examines a log-normal biomarker (reflecting liver function markers such as ALT), and S8 examines a combined location-and-scale difference representing the most common realistic MRCT pattern.
=======
...
>>>>>>> 2c03cea672adb84431201f1e3e0a906e0fce72f9
...
\midrule
<<<<<<< HEAD
S1 & Null & $N(50, 10^2)$ & $N(50, 10^2)$ & 0.000 \\
=======
S1 & Null (identical) & $N(50, 10^2)$ & $N(50, 10^2)$ & 0.000 \\
>>>>>>> 2c03cea672adb84431201f1e3e0a906e0fce72f9
S2 & Location 0.2$\sigma$ & $N(50, 10^2)$ & $N(52, 10^2)$ & 0.074 \\
S3 & Location 0.5$\sigma$ & $N(50, 10^2)$ & $N(55, 10^2)$ & 0.186 \\
S4 & Location 1.0$\sigma$ & $N(50, 10^2)$ & $N(60, 10^2)$ & 0.372 \\
S5 & Scale 1.5$\times$ & $N(50, 10^2)$ & $N(50, 15^2)$ & 0.148 \\
S6 & Shape (Gamma) & $N(50, 10^2)$ & Gamma$(25, 0.5)$ & 0.067 \\
<<<<<<< HEAD
S7 & Skew (log-normal) & $N(50, 10^2)$ & LogN$(\mu_{\ln}, 0.5)$ & 0.302 \\
=======
S7 & Skew (Log-normal) & $N(50, 10^2)$ & LogN$(\mu, \sigma)$ & 0.302 \\
>>>>>>> 2c03cea672adb84431201f1e3e0a906e0fce72f9
...
\begin{tablenotes}
<<<<<<< HEAD
\item The Gamma distribution in S6 has shape parameter 25 and rate 0.5, yielding mean 50 and standard deviation 10.
\item The log-normal distribution in S7 has $\sigma_{\ln} = 0.5$ and $\mu_{\ln} = \ln(50) - \sigma_{\ln}^2/2$, yielding mean $\approx 50$ and CV $\approx 53$\%, consistent with published biological variability for ALT.\cite{sai2021}
\item True nABCD values for S7 and S8 were computed via Monte Carlo integration ($n = 10^6$).
=======
...
\begin{enumerate}[1.]
\item \emph{Bias}: $\text{Mean}(\widehat{\text{nABCD}}) - \text{nABCD}$
\item \emph{Root mean squared error (RMSE)}: $\sqrt{\text{Mean}[(\widehat{\text{nABCD}} - \text{nABCD})^2]}$
...
S5 (Scale) & 0.148 & 0.002 & $-0.012$ & $-0.019$ \\
<<<<<<< HEAD
S6 (Gamma) & 0.067 & 0.028 & 0.003 & $-0.015$ \\
S7 (Log-normal) & 0.302 & 0.019 & 0.009 & 0.005 \\
S8 (Loc.\ + Scale) & 0.175 & 0.024 & 0.011 & 0.006 \\
=======
S6 (Shape) & 0.067 & 0.028 & 0.003 & $-0.015$ \\
...
\end{tabular*}
\begin{tablenotes}
\item Bias is defined as $\text{Mean}(\widehat{\text{nABCD}}) - \text{nABCD}$. Results based on 10{,}000 replications with $B = 2{,}000$ bootstrap resamples.
\end{tablenotes}
\end{table}

<<<<<<< HEAD
For non-null scenarios excluding S4, bias was less than 0.02 in absolute value at $n \geq 100$, indicating satisfactory point estimation performance at practical sample sizes. In particular, the two new scenarios---S7 (log-normal, true nABCD $= 0.302$) and S8 (location + scale, true nABCD $= 0.175$)---showed bias below 0.012 at $n \geq 100$, demonstrating robustness to non-Gaussian and combined distributional differences. For S4 (1.0$\sigma$ location shift), negative bias of approximately $-0.04$ persisted across all sample sizes, reflecting upward bias in the pooled IQR estimator when the location shift is large (Figure~\ref{fig:bias}). These patterns were highly stable across the 10{,}000 replications.

\begin{figure}[t]
\centerline{\includegraphics[width=\textwidth]{fig3_bias.pdf}}
\caption{Bias of nABCD estimator by scenario and sample size. Horizontal dashed lines indicate $\pm 0.02$ bias threshold. For non-null scenarios excluding S4, bias is less than 0.02 at $n \geq 100$.\label{fig:bias}}
\end{figure}

Table~\ref{tab:coverage} presents coverage probabilities of the 95\% percentile bootstrap confidence intervals. For $n \geq 100$, coverage was within 0.87--0.98 for most scenarios, with three notable patterns. First, S3 (0.5$\sigma$), S7 (log-normal), and S8 (location + scale) achieved near-nominal coverage of 0.93--0.95 across all sample sizes, demonstrating excellent performance for moderate distributional differences including non-Gaussian scenarios. Second, S2 showed moderate undercoverage at $n = 50$ (0.669) but recovered to 0.943 at $n = 200$. Third, S4 (1.0$\sigma$ location shift) showed progressive undercoverage with increasing $n$ (0.874 at $n = 100$, 0.740 at $n = 200$), attributable to the persistent negative bias under large distributional differences: the CI narrows with $n$ but remains centered on a biased estimate, causing coverage to deteriorate.

Coverage exhibited a non-monotonic pattern across sample sizes for the shape scenario (S6): 0.576 at $n = 50$, 0.949 at $n = 100$, and 0.997 at $n = 200$. The low coverage at $n = 50$ reflects large positive bias relative to CI width, while the overcoverage at $n = 200$ reflects the combination of shrinking bias ($-0.015$) with CIs that remain relatively wide for this small true value.

We also evaluated BCa (bias-corrected and accelerated) confidence intervals. BCa intervals showed consistently lower coverage than percentile intervals across all scenarios, with the largest discrepancy in scale and shape scenarios (e.g., S6 at $n = 200$: percentile 0.997 vs.\ BCa 0.493; S5 at $n = 100$: percentile 0.979 vs.\ BCa 0.841). The BCa correction overcorrects for nABCD because the statistic is bounded below by zero, causing the acceleration parameter to distort the quantile adjustment. Based on these findings, we adopt percentile bootstrap as the primary inference method.
=======
For most non-null scenarios, bias was less than 0.02 in absolute value at $n \geq 100$, indicating satisfactory point estimation performance at practical sample sizes. For S4 (1.0$\sigma$ location shift), negative bias of approximately $-0.04$ persisted across all sample sizes, reflecting the bounded nature of nABCD near its theoretical upper range (Figure~\ref{fig:bias}). The new scenarios S7 (log-normal) and S8 (location + scale) showed small positive bias that diminished with increasing sample size, following the expected pattern for mid-range true nABCD values.

\begin{figure}[t]
\centerline{\includegraphics[width=\textwidth]{fig3_bias.pdf}}
\caption{Bias of nABCD estimator by scenario and sample size. Horizontal dashed lines indicate $\pm 0.02$ bias threshold. For most scenarios, bias is less than 0.02 at $n \geq 100$, with S4 being the exception due to boundary effects.\label{fig:bias}}
\end{figure}

Table~\ref{tab:coverage} presents coverage probabilities of the 95\% bootstrap confidence intervals. For $n \geq 100$, coverage was within 0.87--0.98 for most scenarios, with S2 showing moderate undercoverage at $n = 100$ (0.896). S4 showed progressive undercoverage with increasing $n$ (0.874 at $n = 100$, 0.740 at $n = 200$), attributable to the persistent negative bias under large distributional differences. At $n = 50$, undercoverage was more pronounced, particularly for S6 (0.576) and S2 (0.669), indicating that this sample size is insufficient for reliable inference.

Coverage exhibited a non-monotonic pattern across sample sizes for the shape scenario (S6): 0.576 at $n = 50$, 0.949 at $n = 100$, and 0.997 at $n = 200$. The low coverage at $n = 50$ reflects large positive bias relative to CI width, while the overcoverage at $n = 200$ reflects the combination of shrinking bias ($-0.015$) with CIs that remain relatively wide for this small true value. The new scenarios S7 (skew) and S8 (location + scale) showed excellent coverage: S7 achieved near-nominal coverage across all sample sizes (0.951--0.954), while S8 showed slight undercoverage that improved with sample size (0.916 at $n=50$ to 0.939 at $n=200$).

We also evaluated BCa (bias-corrected and accelerated) confidence intervals. BCa intervals showed consistently lower coverage than percentile intervals across all scenarios, with the largest discrepancy in scale and shape scenarios (e.g., S5 at $n = 100$: percentile 0.979 vs.\ BCa 0.841; S6 at $n = 200$: percentile 0.997 vs.\ BCa 0.493). The BCa correction overcorrects for nABCD because the statistic is bounded below by zero, causing the acceleration parameter to distort the quantile adjustment. Based on these findings, we adopt percentile bootstrap as the primary inference method.
>>>>>>> 2c03cea672adb84431201f1e3e0a906e0fce72f9
...
S5 (Scale) & 0.962 & 0.979 & 0.933 \\
<<<<<<< HEAD
S6 (Gamma) & 0.576 & 0.949 & 0.997 \\
S7 (Log-normal) & 0.953 & 0.954 & 0.951 \\
S8 (Loc.\ + Scale) & 0.916 & 0.932 & 0.939 \\
=======
S6 (Shape) & 0.576 & 0.949 & 0.997 \\
...
\end{tabular*}
\begin{tablenotes}
\item Coverage under S1 (Null) is not reported because the true value of 0 lies at the boundary of the parameter space.
\item For S4 (large effect), coverage decreased at larger sample sizes due to increased precision revealing the persistent negative bias.
\end{tablenotes}
\end{table}
...
\midrule
S1 (Null) & 0.100 & 0.070 & 0.050 & 0.16 & 0.11 & 0.08 \\
...
S5 (Scale) & 0.045 & 0.035 & 0.031 & 0.19 & 0.13 & 0.09 \\
<<<<<<< HEAD
S6 (Gamma) & 0.045 & 0.024 & 0.023 & 0.16 & 0.11 & 0.08 \\
S7 (Log-normal) & 0.070 & 0.048 & 0.034 & 0.27 & 0.19 & 0.13 \\
S8 (Loc.\ + Scale) & 0.062 & 0.043 & 0.031 & 0.22 & 0.16 & 0.12 \\
=======
S6 (Shape) & 0.045 & 0.024 & 0.023 & 0.16 & 0.11 & 0.08 \\
S7 (Skew) & 0.070 & 0.048 & 0.034 & 0.27 & 0.19 & 0.13 \\
S8 (Loc+Scale) & 0.062 & 0.043 & 0.031 & 0.22 & 0.16 & 0.12 \\
>>>>>>> 2c03cea672adb84431201f1e3e0a906e0fce72f9
\bottomrule
\end{tabular*}
...
S6 (Shape only) & $0.070 \pm 0.024$ & $0.00 \pm 0.14$ & Only nABCD detects \\
<<<<<<< HEAD
...
\begin{enumerate}[1.]
<<<<<<< HEAD
\item \emph{Bias}: Less than 0.02 for non-null scenarios at $n \geq 100$, excluding S4 where persistent bias of $-0.04$ reflects upward bias in pooled IQR at large location shifts
\item \emph{Coverage}: 0.87--0.98 at $n \geq 100$ for most scenarios; near-nominal (0.93--0.95) for S3, S7, and S8
\item \emph{Precision}: RMSE below 0.06 and CI width below 0.19 at $n \geq 100$
\item \emph{Sensitivity}: Captures location, scale, shape, and skewness differences where SMD is limited to location only
\item \emph{Robustness}: New scenarios S7 (log-normal) and S8 (location + scale) confirmed satisfactory performance under non-Gaussian and combined distributional differences
=======
...
\emph{Step 1: Estimate CATE sensitivity $L$ for each EM.}
...
\begin{itemize}
\item \emph{Age}: $L_{\text{age}} \approx 0.01$\% HbA1c per year. Subgroup analyses typically show modest age--treatment interactions, with treatment effect varying by approximately 0.1\% HbA1c per decade of age.
\item \emph{BMI}: $L_{\text{BMI}} \approx 0.02$\% HbA1c per kg/m$^2$. A regression analysis of DPP-4 inhibitor efficacy estimated a BMI coefficient of $-0.02$ per kg/m$^2$ ($p = 0.024$) for the relationship between BMI and HbA1c reduction.\cite{kim2015}
\item \emph{HbA1c}: $L_{\text{HbA1c}} \approx 0.3$\% HbA1c per \% HbA1c. Baseline HbA1c is the strongest effect modifier for most glucose-lowering therapies: meta-regression of 98 DPP-4 inhibitor trials estimated 0.4--0.5 percentage points greater HbA1c reduction per 1\% higher baseline,\cite{craddy2014} and a GLP-1 receptor agonist study estimated $\beta = -0.31$ per \% HbA1c after correcting for regression to the mean.\cite{jones2016}
\end{itemize}

\emph{Step 2: Compute $\Delta_{\max}$ for the most discrepant region pair.}
...
\emph{Step 3: Interpret $\Delta_{\max}$ in clinical context.}
...
<<<<<<< HEAD
The nABCD metric addresses limitations of current approaches. Compared to SMD, nABCD captures full distributional differences including variance and shape. Our simulation demonstrated that nABCD captured scale differences (S5), shape differences (S6), and skewness differences (S7) where SMD estimates remained near zero. Compared to the KS statistic, nABCD provides a direct connection to treatment effect heterogeneity through the heterogeneity bound (equation~\ref{eq:delta_max}), enabling clinical calibration rather than purely statistical assessment. Compared to visual inspection, nABCD is objective and reproducible.
=======
The nABCD metric addresses limitations of current approaches. Compared to SMD, nABCD captures full distributional differences including variance and shape. Our simulation demonstrated that nABCD captured scale differences (S5) and shape differences (S6) where SMD estimates remained near zero. Compared to the KS statistic, nABCD provides a direct connection to treatment effect heterogeneity through the heterogeneity bound (equation~\ref{eq:delta_max}), enabling clinical calibration rather than purely statistical assessment. Compared to visual inspection, nABCD is objective and reproducible.
>>>>>>> 2c03cea672adb84431201f1e3e0a906e0fce72f9

The clinical calibration procedure introduced in Section~\ref{sec:inference} represents a departure from fixed-threshold approaches. Cohen's $d$ benchmarks (small = 0.2, medium = 0.5, large = 0.8) have been widely criticized for context-free application, and we deliberately avoid this pattern. Instead, the heterogeneity bound provides a principled mechanism to translate nABCD into clinically meaningful units. This approach respects the ICH E17 principle that ``similar enough'' is context-dependent: the same nABCD value may support pooling for one EM (where CATE sensitivity is low) but warrant separate analysis for another (where CATE sensitivity is high). Indeed, the application example revealed a \emph{ranking reversal}: BMI had the largest distributional difference (nABCD $= 0.51$) yet the smallest clinical impact ($\Delta_{\max} = 0.16$\%), while baseline HbA1c had a moderate distributional difference (nABCD $= 0.27$) but the largest clinical impact ($\Delta_{\max} = 0.24$\%) due to its 15-fold higher CATE sensitivity. This reversal underscores that distributional distance and clinical relevance are fundamentally different dimensions, and that nABCD magnitude labels (Table~\ref{tab:benchmarks}) describe distributional distance only, not clinical significance. This connection draws on transportability theory\cite{pearl2011,bareinboim2016} to relate distributional distance to potential treatment effect heterogeneity.
...
<<<<<<< HEAD
\item Positive bias under the null at small sample sizes ($n < 100$) can inflate nABCD estimates, and persistent negative bias at large true values (S4, nABCD $= 0.372$) leads to progressive undercoverage (0.740 at $n = 200$). This negative bias arises because the pooled IQR estimator is upward-biased when the two distributions are substantially separated, causing the ratio $W_1 / (2 \cdot \text{IQR})$ to be systematically underestimated. When the true value equals zero, the parameter lies on the boundary of the parameter space, where standard bootstrap consistency results may not apply;\cite{andrews2000} we therefore do not report null-scenario coverage and recommend interpreting nABCD estimates near zero with caution. For non-null scenarios, the true value lies in the interior of the parameter space where standard bootstrap consistency holds, and our simulation confirms this with coverage of 0.87--0.98 at $n \geq 100$ for most scenarios. Coverage can also exhibit non-monotonic behavior: for the shape scenario (S6), coverage was 0.576 at $n = 50$, 0.949 at $n = 100$, and 0.997 at $n = 200$, reflecting how the balance between bias magnitude and CI width shifts with sample size. We use the percentile bootstrap, which is first-order accurate; bias-corrected methods such as BCa showed consistently inferior coverage across all eight scenarios (Section~\ref{sec:sim_results}), likely because the acceleration parameter is destabilized by the bounded, non-negative nature of nABCD. The studentized bootstrap would require variance estimation for the Wasserstein distance ratio, which adds substantial complexity. We recommend $n \geq 100$ per region for reliable point estimation and confidence intervals, and caution that coverage may degrade when the true nABCD exceeds approximately 0.3.
=======
\item Positive bias under the null at small sample sizes ($n < 100$) can inflate nABCD estimates, and persistent negative bias at large true values (S4, nABCD $= 0.372$) leads to progressive undercoverage (0.740 at $n = 200$). These boundary effects are inherent to the non-negative, bounded nature of nABCD as a function of the Wasserstein distance. When the true value equals zero, the parameter lies on the boundary of the parameter space, where standard bootstrap consistency results may not apply;\cite{andrews2000} we therefore do not report null-scenario coverage and recommend interpreting nABCD estimates near zero with caution. For non-null scenarios, the true value lies in the interior of the parameter space where standard bootstrap consistency holds, and our simulation confirms this with coverage of 0.87--0.98 at $n \geq 100$ for most scenarios; notably, the new scenarios S7 (skew) and S8 (location + scale) achieved coverage between 0.93--0.95 across all sample sizes. Coverage can also exhibit non-monotonic behavior: for the shape scenario (S6), coverage was 0.576 at $n = 50$, 0.949 at $n = 100$, and 0.997 at $n = 200$, reflecting how the balance between bias magnitude and CI width shifts with sample size. We use the percentile bootstrap, which is first-order accurate; bias-corrected methods such as BCa showed inferior performance for this bounded statistic (Section~\ref{sec:sim_results}), and the studentized bootstrap would require variance estimation for the Wasserstein distance ratio, which adds substantial complexity. We recommend $n \geq 100$ per region for reliable point estimation and confidence intervals, and caution that coverage may degrade when the true nABCD exceeds approximately 0.3.
>>>>>>> 2c03cea672adb84431201f1e3e0a906e0fce72f9
