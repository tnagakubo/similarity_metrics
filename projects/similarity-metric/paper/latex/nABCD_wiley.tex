...
A key strategy for enhancing the ability to asess regional consistency in treatment outcomes is the regional pooling approach, wherein regions with similar patient characteristics are grouped for analysis.\cite{iche17} The ICH E17 guideline explicitly recommends that pooling decisions be based on the similarity of effect modifier (EM) distributions:
...
\begin{enumerate}[1.]
\item \emph{Bias}: $\text{Mean}(\widehat{\text{nABCD}}) - \text{true nABCD}$
\item \emph{Root mean squared error (RMSE)}: $\sqrt{\text{Mean}[(\widehat{\text{nABCD}} - \text{true nABCD})^2]}$
