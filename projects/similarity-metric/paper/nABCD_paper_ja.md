...
表6にRMSEと平均CI幅をシナリオ別に示す。RMSEは $n$ の増加に伴い一貫して減少し、$n = 200$ ですべてのシナリオで0.06未満の値に達した。平均CI幅は比例して狭まり、$n = 100$ のCIは典型的に0.11–0.19 nABCD単位にまたがる。臨床較正フレームワークでは、このCI幅は $\Delta_{\max}$ に伝播する：例えば、$L = 0.3$ かつ IQR = 1.5 の場合、nABCDのCI幅0.13は $0.3 \times 1.5 \times 0.13 = 0.12$% HbA1cのCI幅に対応する——意味のある臨床較正に十分な精度水準である。
...
シナリオS3（$0.5\sigma$ の位置シフト、真のnABCD = 0.186）は、臨床的に関連する効果量での操作特性を例示する：バイアスは無視できる（$n = 100$ で −0.002）、カバレッジは名目どおり（0.949）、CI幅（0.18）は $L = 0.3$、IQR = 1.5 で較正すると $0.3 \times 1.5 \times 0.18 = 0.16$% HbA1cの $\Delta_{\max}$ CI幅に変換され——情報に基づく規制審議に十分な精度水準である。
...
同時適格パートナーの中で、R4が単一プーリング候補（single-pool candidate）として最も有力な候補として浮上する。R4は年齢で3番目に低く（0.016）、SBPで4番目に低く（0.042）、両修飾因子にわたってバランスがとれている。R5は年齢nABCDが最低（0.011）であるが、SBP nABCDは6番目に低い（0.056）にとどまり、非対称な profile を示す。本フレームワークは、スポンサーが臨床的・規制的助言者と協議する際の判断を支援する定量的入力——表\ref{tab:gusto_nabcd}のブートストラップCIつきnABCD、表\ref{tab:gusto_lstar_joint}のパートナー別 $L^*$、および仮定された上界の下での同時適格性——を供給する。なお、GUSTO-Iデータは1990–1993年に収集されたものであり、本適用事例は、公開IPDを用いた方法論的例示として提示するものであって、これら特定の分布を現代の血栓溶解薬試験計画に対する根拠として推奨するものではない。
