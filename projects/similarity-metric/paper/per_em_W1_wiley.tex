...
% TODO (2026-08-30): several sentences below are deliberately written at the
% level the current manuscript supports. When the decision-performance
% simulation (outline Study 2) enters Section 3, re-point them: (a) the
% "constructible but does not manifest" sentence can then cite the
% matched-moment and displaced-extreme worlds directly; (b) the RV2/RV3
% paragraph can add "consistent with the simulation finding that added moments
% are not free"; (c) the rho paragraph can cite the rho/rho_trans regime.
The preceding subsections applied the proposed framework alone. To place its selection in context, we repeated the complete partner-selection exercise with the alternative measures a sponsor could have used at the planning stage: the KS statistic and SMD of Section~\ref{sec:existing}, and three representative-value distances denoted RV1--RV3. RV1 is the representative-value distance of the pooling strategy of Komiyama et al., which describes each region by one representative value per effect modifier---for a continuous modifier, the regional mean---and measures the distance between the resulting summaries.\cite{komiyama2024} RV2 and RV3 are our extensions, not part of the cited proposal: they augment the coordinate vector with the regional standard deviation (RV2) and additionally the skewness (RV3), probing whether added distributional summaries close the gap to full-distribution measures. KL divergence is omitted for the reasons given in Section~\ref{sec:existing}. Because no alternative measure has a pathway from a clinical margin to a decision threshold, none can be given its own $\tau$; instead each measure admits its $k$ closest partners, where $k$ is the number $W_1$ admits at $\tau_{\text{clin}}$ ($k = 9$ on age, $k = 11$ on SBP). Matching the count removes any advantage or penalty from admitting more or fewer partners.
