...
% pluralized to match.
The remainder of this paper is organized as follows. Section~\ref{sec:methods} presents the methodological framework: the definition of the per-EM $W_1$ distance, its clinical calibration, the estimation procedure, and an operability check. Section~\ref{sec:simulation} presents two simulation studies, evaluating decision performance and estimation properties. Section~\ref{sec:application} demonstrates the framework using publicly available individual patient data. Section~\ref{sec:discussion} discusses implications, limitations, and future directions.
...
Specifying $L_{\text{clinical}}$ (or its upper bound $L_{\text{UB}}$) is a clinical judgment, informed but not determined by prior evidence, for which the sponsor carries responsibility---the same footing as a non-inferiority margin. Writing $R_a(x)$ for the risk under arm $a$, $\tau(x) = R_1(x) - R_0(x)$, so $|\tau'(x)| \leq |R_1'(x)| + |R_0'(x)|$: arm-level risk gradients constrain the modification slope only jointly, and a prognostic gradient alone guarantees no bound, prognostic and treatment-effect gradients being distinct quantities.\cite{ballman2015,kent2020} For the GUSTO-I case study presented in Section~\ref{sec:application}, the Fibrinolytic Therapy Trialists' meta-analysis\cite{ftt1994} and GUSTO-I subgroup analyses\cite{gusto1993,lee1995} inform illustrative bounds of $L_{\text{UB,age}} \approx 1 \times 10^{-2}$ per year and $L_{\text{UB,SBP}} \approx 2 \times 10^{-3}$ per mmHg on the 30-day mortality scale; these exceed the empirical prognostic age gradient of approximately $6 \times 10^{-3}$ per year reported by Lee et al.,\cite{lee1995} a reference point rather than a guarantee. Conclusions drawn from the bounds are conditional on them, and the sensitivity analysis of Section~\ref{sec:app_clinical}, which reports how the partner selection changes as each bound is varied from 0.8 to 2 times its stated value, is the primary robustness statement accompanying this judgment. Supplement~D discusses the derivation of these bounds, alternative specification strategies such as the deft estimator of Fisher et al.,\cite{fisher2017} and the limitations of class-level evidence. The methodology does not require an exact $L_{\text{clinical}}$; comparison of $L^*$ to $L_{\text{UB}}$ accommodates the typical uncertainty in this parameter.
...
% DECIDED (Tak, 2026-09-05): the rho paragraph below stays as written -- the
% rho/rho_trans regime is not written into Section 3 (see the decision note at
% the end of sec:study2_clustering), so no regime citation is added here.
% (Items (a) and (b) of the original note were already resolved on 2026-08-30
% when Study 2 entered Section 3.)
