...
bias_df <- data.frame(
  Scenario = rep(c("S1 (Null)", "S2 (0.2σ)", "S3 (0.5σ)", "S4 (1.0σ)",
                    "S5 (Scale)", "S6 (Skew)", "S7 (Loc+Scale)"), each = 3),
  True_nABCD = rep(c(0.000, 0.073, 0.180, 0.328, 0.122, 0.304, 0.175), each = 3),
  n = rep(c(50, 100, 200), 7),
  Bias = c(0.093, 0.066, 0.047,  # S1
           0.039, 0.020, 0.009,  # S2
           0.010, 0.003, 0.001,  # S3
           0.006, 0.003, 0.001,  # S4
           0.028, 0.014, 0.007,  # S5
           0.015, 0.008, 0.003,  # S6
           0.023, 0.011, 0.006)  # S7
)
...
coverage_df <- data.frame(
  ...
  Coverage = c(0.652, 0.881, 0.941,  # S2
               ...
               0.953, 0.954, 0.954,  # S6
               0.917, 0.935, 0.944)  # S7
)
...
precision_df <- data.frame(
  Scenario = rep(c("S1 (Null)", "S2 (0.2σ)", "S3 (0.5σ)", "S4 (1.0σ)",
                    "S5 (Scale)", "S6 (Skew)", "S7 (Loc+Scale)"), each = 3),
  n = rep(c(50, 100, 200), 7),
  RMSE = c(0.099, 0.069, 0.049,  # S1
           0.061, 0.044, 0.032,  # S2
           0.067, 0.048, 0.035,  # S3
           0.062, 0.043, 0.030,  # S4
           0.053, 0.036, 0.025,  # S5
           0.070, 0.049, 0.034,  # S6
           0.062, 0.043, 0.030), # S7
  CI_Width = c(0.16, 0.11, 0.08,  # S1
               0.18, 0.13, 0.11,  # S2
               0.23, 0.18, 0.13,  # S3
               0.24, 0.17, 0.12,  # S4
               0.19, 0.13, 0.09,  # S5
...
smd_df <- data.frame(
  Scenario = c("S3 (Location)", "S5 (Scale)", "S6 (Skew)"),
  nABCD_mean = c(0.183, 0.136, 0.312),
  nABCD_sd = c(0.048, 0.033, 0.048),
  SMD_mean = c(0.50, 0.00, 0.00),
  SMD_sd = c(0.14, 0.14, 0.14)
)
...
bias_df$Scenario <- factor(bias_df$Scenario,
  levels = c("S1 (Null)", "S2 (0.2σ)", "S3 (0.5σ)", "S4 (1.0σ)",
             "S5 (Scale)", "S6 (Skew)", "S7 (Loc+Scale)"))

# Color: boundary scenarios in red, others in blue
bias_df$Type <- ifelse(bias_df$Scenario %in% c("S1 (Null)", "S2 (0.2σ)"),
...
smd_long <- smd_df |>
  ...
  mutate(
    ...
    Scenario = factor(Scenario, levels = c("S3 (Location)", "S5 (Scale)", "S6 (Skew)"))
  )
