...
\begin{figure}[t]
\centerline{\includegraphics[width=0.8\textwidth]{fig1_nabcd_definition.pdf}}
\caption{nABCD as the area between cumulative distribution functions, illustrated with a right-skewed Region~1 (gamma distribution) and a symmetric Region~2 (normal distribution). Panel~(A): probability densities. Panel~(B): cumulative distribution functions; the shaded area equals the Wasserstein-1 distance $W_1(F_1, F_2)$.\label{fig:nabcd_definition}}
\end{figure}

The \emph{normalized Area Between Cumulative Distributions} (nABCD) is defined as the Wasserstein-1 distance normalized by the pooled interquartile range:
\begin{equation}
\text{nABCD}(F_1, F_2) = \frac{W_1(F_1, F_2)}{\text{IQR}_{\text{pooled}}},
\label{eq:nabcd}
\end{equation}
where $\text{IQR}_{\text{pooled}} = Q_{0.75}(F_{\text{pool}}) - Q_{0.25}(F_{\text{pool}})$ with $F_{\text{pool}} = \lambda F_1 + (1-\lambda) F_2$ and $\lambda = n_1/(n_1+n_2)$. The denominator normalizes $W_1$ by a scale estimator to achieve scale-free interpretation. We adopt the interquartile range (IQR) for its direct interpretability as the central 50\% spread of the effect modifier distribution. nABCD is calibrated so that a one-IQR location shift yields nABCD $= 1.0$.
...
\begin{equation}
|\bar{\tau}_1 - \bar{\tau}_2| \leq L \cdot \text{IQR}_{\textup{pooled}} \cdot \text{nABCD}(F_1, F_2).
\label{eq:heterogeneity_nabcd}
\end{equation}
\end{proposition}

\begin{proof}
Substituting (\ref{eq:nabcd}) into (\ref{eq:heterogeneity_bound}) yields $|\bar{\tau}_1 - \bar{\tau}_2| \leq L \cdot W_1(F_1, F_2) = L \cdot \text{IQR}_{\text{pooled}} \cdot \text{nABCD}(F_1, F_2)$.
\end{proof}
...
\begin{equation}
\hnABCD = \frac{\sum_{k=1}^{n_1+n_2-1} |\hat{F}_1(x_{(k)}) - \hat{F}_2(x_{(k)})| \cdot (x_{(k+1)} - x_{(k)})}{\widehat{\text{IQR}}_{\text{pooled}}},
\label{eq:estimator}
\end{equation}
...
\item Derive the bootstrap CI for $\Delta_{\max}$ from the nABCD confidence interval: $[\Delta_{\max,L},\, \Delta_{\max,U}] = L \cdot \text{IQR}_{\text{pooled}} \cdot [\text{nABCD}_{L},\, \text{nABCD}_{U}]$. This expresses inferential uncertainty directly on the clinical scale.
...
Table~\ref{tab:bias} presents the bias of the nABCD estimator across scenarios and sample sizes. The estimator showed positive bias under the null hypothesis (S1), with bias decreasing from 0.185 at $n=50$ to 0.094 at $n=200$. This positive bias arises because the non-negativity constraint inherited from the Wasserstein distance yields $\widehat{\text{nABCD}} > 0$ with probability 1 in finite samples, even when the true value equals zero ($F_1 = F_2$ almost everywhere). The estimator remains consistent ($\widehat{\text{nABCD}} \to 0$ as $n \to \infty$), but the convergence is slow near the parameter space boundary.
...
S6 (Skew only) & $0.622 \pm 0.094$ & $0.00 \pm 0.14$ & Only nABCD detects \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\item SMD depends only on the difference in means, making it insensitive to differences in variance and skewness. nABCD captures the full distributional difference. S6 is particularly informative because a large distributional difference (nABCD $= 0.62$) is entirely invisible to SMD.
\end{tablenotes}
\end{table}
...
\begin{enumerate}[1.]
\item \emph{Bias}: Less than 0.04 for most non-null scenarios at $n \geq 100$, with positive bias largest near the boundary (S1: 0.132 at $n = 100$) and negligible for scenarios with true nABCD above 0.2 (S3: 0.009, S4: 0.006 at $n = 100$)
\item \emph{Coverage}: 0.88--0.96 at $n \geq 100$ for scenarios with true nABCD $\geq 0.2$ (S3--S7), and S6 (skew) and S7 (location + scale) achieved coverage between 0.91--0.95
\item \emph{Precision}: RMSE below 0.12 and CI width below 0.38 at $n \geq 100$
\item \emph{Sensitivity}: Captures location, scale, and skewness differences where SMD is limited to location only
\end{enumerate}

Scenario S3 (0.5$\sigma$ location shift, true nABCD $= 0.358$) exemplifies the operating characteristics at clinically relevant effect sizes. Bias was negligible ($+0.009$ at $n = 100$), coverage was nominal (0.945), and CI width (0.36) translated to a $\Delta_{\max}$ CI width of $0.01 \times 17 \times 0.36 = 0.061$ (6.1\%pt) when calibrated with $L = 0.01$/yr and IQR $= 17$ (age-scale parameters representative of the GUSTO-I application)---a level of precision sufficient for informed regulatory deliberation.
...
\midrule
R1  & 2188 & 0.035 [0.028, 0.056] & 0.116 [0.087, 0.158] \\
R2  & 2952 & 0.122 [0.089, 0.157] & 0.030 [0.023, 0.059] \\
R3  & 2030 & 0.151 [0.114, 0.191] & 0.102 [0.070, 0.148] \\
R4  & 2876 & 0.031 [0.021, 0.065] & 0.084 [0.058, 0.121] \\
R5  & 1909 & 0.022 [0.020, 0.052] & 0.112 [0.077, 0.154] \\
R6  & 1585 & 0.053 [0.037, 0.081] & 0.099 [0.063, 0.152] \\
R7  & 3150 & 0.022 [0.016, 0.052] & 0.164 [0.127, 0.196] \\
R9  & 3123 & 0.033 [0.023, 0.064] & 0.219 [0.182, 0.260] \\
R10 & 1717 & 0.076 [0.044, 0.117] & 0.125 [0.090, 0.170] \\
R11 & 2491 & 0.070 [0.042, 0.104] & 0.208 [0.164, 0.244] \\
R12 & 4352 & 0.048 [0.027, 0.079] & 0.194 [0.166, 0.233] \\
R13 & 2297 & 0.067 [0.045, 0.102] & 0.075 [0.046, 0.122] \\
R14 & 3437 & 0.040 [0.024, 0.073] & 0.128 [0.104, 0.163] \\
R15 & 2576 & 0.034 [0.022, 0.069] & 0.148 [0.113, 0.187] \\
R16 & 1231 & 0.101 [0.067, 0.143] & 0.143 [0.102, 0.190] \\
\bottomrule
\end{tabular}
\begin{tablenotes}
...
\caption{Forest plots of nABCD point estimates with 95\% percentile bootstrap CIs for age (A) and systolic blood pressure (B). Partners are ordered by ascending nABCD within each panel; ordering and horizontal scale differ between panels.}
\label{fig:gusto_forest}
\end{figure}

The two candidate effect modifiers produce different patterns of distributional similarity. For age, partner-region nABCD point estimates span a narrow range, from 0.022 (R5, R7) at the low end up to 0.151 (R3) at the high end, and eleven of the fifteen partners have age nABCD below 0.080. For SBP, point estimates span a wider range, from 0.030 (R2) up to 0.219 (R9), with the majority of partners falling in the 0.100--0.220 interval. SBP distributions therefore exhibit greater between-region heterogeneity than age distributions across GUSTO-I, and the ranking of partners by each candidate effect modifier differs accordingly.

Several partners fall in the middle of each ranking where uncertainty intervals overlap substantially. R16's age nABCD ($0.101$) is the 13th-smallest value with 95\% CI $[0.067, 0.143]$, placing it in a region where its relative position is not sharply determined by the data---the CI spans the range covered by several partners with smaller point estimates. R6's age nABCD ($0.053$) is the 9th-smallest value but its SBP nABCD of $0.099$ sits near the lower end of the SBP-nABCD distribution with 95\% CI $[0.063, 0.152]$, similarly spanning a broad region of the SBP ranking. These mid-rank cases illustrate the value of reporting bootstrap CIs alongside point estimates. The ranking itself is accompanied by an uncertainty interval that informs how confidently one partner can be placed above or below another.

An instructive contrast emerges between R2 and R9. R2 has the second-largest age nABCD (0.122) but the smallest SBP nABCD (0.030), while R9 has the 4th-smallest age nABCD (0.033) but the largest SBP nABCD (0.219). A ranking based on either candidate effect modifier alone would reach opposite conclusions about these two regions. This asymmetry underscores the necessity of evaluating all candidate effect modifiers jointly.

\subsection{Clinical Interpretation and Pooling Candidates}\label{sec:app_clinical}

We require an upper bound on the CATE sensitivity $L$ for each candidate effect modifier; recall that $L$ governs how strongly treatment effect varies with the modifier. For thrombolysis in acute myocardial infarction, class-level meta-analytic evidence (GUSTO-I,\cite{gusto1993} FTT collaborative meta-analysis\cite{ftt1994}) suggests that age-related and SBP-related treatment effect modification on 30-day mortality falls within approximately 10\%pt per decade and 2\%pt per 10~mmHg, respectively. These are intended as upper plausibility envelopes rather than central effect estimates. We adopt them as illustrative bounds: $L_{\text{age}} = 1\times10^{-2}$ /yr and $L_{\text{SBP}} = 2\times10^{-3}$ /mmHg.

For each partner region, we compare the region-specific $L^*$ value (Table~\ref{tab:gusto_lstar_joint}) with the corresponding plausible upper bound (denoted $L_{\text{UB}}$). A region is judged eligible for pooling on a given effect modifier if $L^* > L_{\text{UB}}$, meaning the CATE sensitivity required for the observed distributional difference to produce $\Delta_{\text{clin}} = 1$\%pt exceeds the plausible bound. On age, the eligible partners are R1, R4, R5, R6, R7, R9, R12, R14, R15 (nine regions with $L^*_{\text{age}} > 0.01$). On SBP, the eligible partners are R1, R4, R5, R6, R13, R14, R15 (seven regions with $L^*_{\text{SBP}} > 0.002$); R7 ($L^*_{\text{SBP}} = 0.0020 = L_{\text{UB}}$) is excluded.

\begin{table}[ht]
\centering
\caption{Clinical interpretation ($L^*$ at $\Delta_{\text{clin}} = 1$\%pt) and joint eligibility for all 15 partner regions, listed by region number. $L^*$ is the minimum CATE sensitivity (per year for age, per mmHg for SBP) required for the observed distributional difference to produce a treatment effect difference of $1$\%pt on the absolute 30-day mortality scale. Eligibility is judged $L^* > L_{\text{UB}}$ with $L_{\text{age},\text{UB}} = 1\times10^{-2}$ /yr and $L_{\text{SBP},\text{UB}} = 2\times10^{-3}$ /mmHg.\label{tab:gusto_lstar_joint}}
\begin{tabular}{lrrrrccc}
\toprule
\textbf{Partner} & \textbf{nABCD\textsubscript{age}} & \textbf{nABCD\textsubscript{SBP}} & \textbf{$L^*$ age (/yr)} & \textbf{$L^*$ SBP (/mmHg)} & \textbf{Age elig.} & \textbf{SBP elig.} & \textbf{Joint} \\
\midrule
R1  & 0.035 & 0.116 & 0.0155 & 0.0028 & \checkmark & \checkmark & \checkmark \\
R2  & 0.122 & 0.030 & 0.0047 & 0.0105 & --- & \checkmark & --- \\
R3  & 0.151 & 0.102 & 0.0038 & 0.0030 & --- & \checkmark & --- \\
R4  & 0.031 & 0.084 & 0.0180 & 0.0038 & \checkmark & \checkmark & \checkmark \\
R5  & 0.022 & 0.112 & 0.0256 & 0.0030 & \checkmark & \checkmark & \checkmark \\
R6  & 0.053 & 0.099 & 0.0110 & 0.0030 & \checkmark & \checkmark & \checkmark \\
R7  & 0.022 & 0.164 & 0.0247 & 0.0020 & \checkmark & --- & --- \\
R9  & 0.033 & 0.219 & 0.0171 & 0.0015 & \checkmark & --- & --- \\
R10 & 0.076 & 0.125 & 0.0072 & 0.0025 & --- & \checkmark & --- \\
R11 & 0.070 & 0.208 & 0.0079 & 0.0016 & --- & --- & --- \\
R12 & 0.048 & 0.194 & 0.0116 & 0.0016 & \checkmark & --- & --- \\
R13 & 0.067 & 0.075 & 0.0087 & 0.0042 & --- & \checkmark & --- \\
R14 & 0.040 & 0.128 & 0.0142 & 0.0023 & \checkmark & \checkmark & \checkmark \\
R15 & 0.034 & 0.148 & 0.0164 & 0.0021 & \checkmark & \checkmark & \checkmark \\
R16 & 0.101 & 0.143 & 0.0057 & 0.0023 & --- & \checkmark & --- \\
\bottomrule
\end{tabular}
\begin{tablenotes}
...
Among the jointly eligible partners, R4 emerges as the leading single-pool candidate. R4 ranks 3rd-lowest on age (0.031) and 4th-lowest on SBP (0.084), balanced across both modifiers. R5 has the lowest age nABCD (0.022) but the 6th-lowest SBP nABCD (0.112), an asymmetric profile. The framework supplies the quantitative inputs (nABCD with bootstrap CIs in Table~\ref{tab:gusto_nabcd}, partner-specific $L^*$ in Table~\ref{tab:gusto_lstar_joint}, and joint eligibility under the assumed bounds) to support sponsor judgment in consultation with clinical and regulatory advisors. Note that the GUSTO-I data were collected during 1990--1993; the application here serves as methodological illustration using publicly available IPD, not as an endorsement of these specific distributions for contemporary thrombolytic-trial planning.
