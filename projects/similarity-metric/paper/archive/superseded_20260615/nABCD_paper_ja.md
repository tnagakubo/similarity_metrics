...
**正規化累積分布間面積**（nABCD）は、Wasserstein-1距離をプールされた四分位範囲で正規化したものとして定義される：

$$\text{nABCD}(F_1, F_2) = \frac{W_1(F_1, F_2)}{\text{IQR}_{\text{pooled}}} \tag{4}$$

ここでプールIQRは結合サンプルから計算される。IQRに基づく正規化は、**スケールフリーな解釈**を可能にし、外れ値に頑健であり、分布差を散布度の単位で表現する。nABCDは較正されており、1-IQRの位置シフトにより nABCD = 1.0 となる。
...
$$|\bar{\tau}_1 - \bar{\tau}_2| \leq L \cdot \text{IQR}_{\text{pooled}} \cdot \text{nABCD}(F_1, F_2) \tag{5}$$

**証明**: 式(4)を式(2)に代入すると、$|\bar{\tau}_1 - \bar{\tau}_2| \leq L \cdot W_1(F_1, F_2) = L \cdot \text{IQR}_{\text{pooled}} \cdot \text{nABCD}(F_1, F_2)$ が得られる。

### 2.3 推定

地域1からのサンプル $\{X_{1,i}\}_{i=1}^{n_1}$ および地域2からのサンプル $\{X_{2,j}\}_{j=1}^{n_2}$ が与えられたとき、nABCDは経験分布関数を用いて推定される：

$$\widehat{\text{nABCD}} = \frac{\sum_{k=1}^{n_1+n_2-1} |\hat{F}_1(x_{(k)}) - \hat{F}_2(x_{(k)})| \cdot (x_{(k+1)} - x_{(k)})}{\widehat{\text{IQR}}_{\text{pooled}}} \tag{6}$$
...
$$\Delta_{\max} = L \cdot \text{IQR}_{\text{pooled}} \cdot \text{nABCD}(F_1, F_2) \tag{7}$$
...
4. nABCD信頼区間から $\Delta_{\max}$ のブートストラップCIを導出する：$[\Delta_{\max,L},\, \Delta_{\max,U}] = L \cdot \text{IQR}_{\text{pooled}} \cdot [\text{nABCD}_{L},\, \text{nABCD}_{U}]$。これは推論的不確実性を臨床スケール上で直接表現する。
...
| nABCD範囲 | 解釈 | 推奨アクション |
|-----------|------|----------------|
| < 0.10 | 無視できる差 (Negligible) | プーリングは広く支持可能 |
| 0.10–0.30 | 小さな差 (Small) | プーリングは一般的に許容可能 |
| 0.30–0.60 | 中程度の差 (Moderate) | 臨床較正を実施 |
| > 0.60 | 大きな差 (Large) | 臨床較正が必須 |
...
| S5 | スケール $1.5\times$ | $N(50, 10^2)$ | $N(50, 15^2)$ | 0.244 |
| S6 | 歪み（対数正規） | $N(50, 10^2)$ | LogN$(\mu, \sigma)$ | 0.606 |
| S7 | 位置＋スケール | $N(50, 10^2)$ | $N(55, 15^2)$ | 0.347 |
...
表4にnABCD推定量のバイアスをシナリオおよびサンプルサイズ別に示す。推定量は帰無仮説下（S1）で正のバイアスを示し、バイアスは $n=50$ の0.185から $n=200$ の0.094に減少した。この正のバイアスはWasserstein距離の非負性に起因する：真のnABCDがゼロであっても、サンプリング変動が正の推定値を生成する。

**表4: シナリオおよびサンプルサイズ別のnABCD推定量のバイアス**

| シナリオ | 真のnABCD | バイアス $n$=50 | バイアス $n$=100 | バイアス $n$=200 |
|----------|-----------|:---:|:---:|:---:|
| S1（帰無） | 0.000 | +0.185 | +0.132 | +0.094 |
| S2（$0.2\sigma$） | 0.146 | +0.078 | +0.038 | +0.017 |
| S3（$0.5\sigma$） | 0.358 | +0.023 | +0.009 | +0.005 |
| S4（$1.0\sigma$） | 0.656 | +0.014 | +0.006 | +0.003 |
| S5（スケール） | 0.244 | +0.056 | +0.028 | +0.014 |
| S6（歪み） | 0.606 | +0.034 | +0.016 | +0.010 |
| S7（位置+スケール） | 0.347 | +0.049 | +0.027 | +0.015 |

> バイアスは $\text{Mean}(\widehat{\text{nABCD}}) - \text{nABCD}$ として定義。$B = 2{,}000$ ブートストラップリサンプルによる10,000反復に基づく結果。

真のnABCDが約0.2を超えるシナリオでは、$n \geq 100$ でバイアスの絶対値は0.04未満であり、実用的なサンプルサイズでの十分な点推定性能を示す（図2(A)参照）。S4（$1.0\sigma$ の位置シフト）では、バイアスは無視できる程度（$n = 50$ で0.014、$n = 200$ で0.003）であり、大きな分布差に対する正確な推定を確認した。境界近傍シナリオ（S1、S2）では正のバイアスがより顕著であり、サンプルサイズに伴って緩やかに減少する——真値がゼロに近い場合にサンプリング変動が推定値を上方に押し上げる、非負統計量の周知の現象である。

表5に95%ブートストラップ信頼区間のカバレッジ確率を示す。真のnABCDが約0.2を超えるシナリオでは、$n \geq 100$ でカバレッジはほぼ名目どおりであった：S3（0.945–0.946）、S4（0.954–0.955）、S6（0.951–0.954）、S7（0.932–0.938）。S4（$1.0\sigma$ の位置シフト）はすべてのサンプルサイズで特に安定したカバレッジを示し、大きな分布差においても十分なブートストラップ性能を確認した。S5（スケール）は $n=50$ で中程度のアンダーカバレッジ（0.847）を示したが、$n=200$ ではほぼ名目どおりに改善（0.939）した。S2（小さな位置シフト）は境界近傍の真値のため $n=50$ でアンダーカバレッジ（0.652）を示したが、$n=200$ で0.943まで改善した。
...
| S5（スケール） | 0.847 | 0.910 | 0.939 |
| S6（歪み） | 0.954 | 0.954 | 0.951 |
| S7（位置+スケール） | 0.911 | 0.932 | 0.938 |

> S1（帰無）のカバレッジは、真値0がパラメータ空間の境界上にあるため報告しない。S4ではすべてのサンプルサイズでカバレッジがほぼ名目どおり（0.947–0.955）であり、大きな分布差においても十分な性能を確認した。S2の小さなサンプルサイズ（$n=50$ で0.652）およびS5（$n=50$ で0.847）におけるアンダーカバレッジは、$n$ の増加とともに減少する境界近接効果を反映する。$F_1 = F_2$ では $W_1$ のHadamard微分が非線形となりブートストラップの一致性に影響しうる (Sommerfeld & Munk 2018) が、この境界挙動はサンプルサイズの増加と分布的隔たりの増加に伴い減少し、$n \geq 100$ でS3–S7にほぼ名目どおりのカバレッジが観察されたことで確認される。

#### 3.2.2 推定精度

推定品質のシナリオおよびサンプルサイズにわたる要約を、カバレッジ確率と平均CI幅の両方で示す（図3参照）。

表6にRMSEと平均CI幅をシナリオ別に示す。RMSEは $n$ の増加に伴い一貫して減少し、$n = 200$ ですべてのシナリオで0.10未満の値に達した。平均CI幅は比例して狭まり、$n = 100$ のCIは典型的に0.22–0.38 nABCD単位にまたがる。臨床較正フレームワークでは、このCI幅は $\Delta_{\max}$ に伝播する：例えば、GUSTO-I適用における年齢を代表する値として $L = 0.01$/yr かつ IQR $= 17$ を用いる場合、nABCDのCI幅0.27は $0.01 \times 17 \times 0.27 = 0.046$（4.6\%pt）の $\Delta_{\max}$ CI幅に対応する——意味のある臨床較正に十分な精度水準である。

**表6: シナリオおよびサンプルサイズ別のRMSEと平均CI幅**

| シナリオ | RMSE $n$=50 | RMSE $n$=100 | RMSE $n$=200 | CI幅 $n$=50 | CI幅 $n$=100 | CI幅 $n$=200 |
|----------|:-:|:-:|:-:|:-:|:-:|:-:|
| S1（帰無） | 0.20 | 0.14 | 0.10 | 0.33 | 0.22 | 0.16 |
...
| S5（スケール） | 0.11 | 0.07 | 0.05 | 0.37 | 0.26 | 0.19 |
| S6（歪み） | 0.14 | 0.10 | 0.07 | 0.54 | 0.38 | 0.26 |
| S7（位置+スケール） | 0.12 | 0.09 | 0.06 | 0.43 | 0.32 | 0.23 |

> CI幅は10,000反復にわたる（上限 − 下限）の平均として定義。

帰無仮説下（S1）では、正のバイアスによりCIがゼロから上方にシフトする。$n = 50$ で平均推定値は0.185、平均CI幅は0.33であった；$n = 200$ では平均推定値は0.094に減少し、平均CI幅は0.16であった。このバイアスは臨床較正に関連する：真の分布差がゼロのとき、推定量は小さな正の $\Delta_{\max}$ を示唆する。実務者はこの保守的傾向に留意すべきであり、特に小さなサンプルサイズでは注意が必要である。
...
|----------|-----------------|---------------|------|
| S3（位置） | $0.367 \pm 0.097$ | $0.50 \pm 0.14$ | 両方検出 |
| S5（スケールのみ） | $0.272 \pm 0.066$ | $0.00 \pm 0.14$ | **nABCDのみ検出** |
| S6（歪みのみ） | $0.622 \pm 0.094$ | $0.00 \pm 0.14$ | **nABCDのみ検出** |

> SMDは平均の差のみに依存するため、分散・歪度の差に対して鈍感である。nABCDは完全な分布差を捕捉する。S6は特に示唆的：大きな分布差（nABCD = 0.62）がSMDには完全に見えない。
...
1. **バイアス**: ほとんどの非帰無シナリオで $n \geq 100$ において0.04未満。境界近傍で正のバイアスが最大（S1：$n=100$ で0.132）、真のnABCDが0.2を超えるシナリオでは無視できる程度（S3：0.009、S4：0.006、いずれも $n=100$）
2. **カバレッジ**: 真のnABCD $\geq 0.2$（S3–S7）のシナリオでは $n \geq 100$ において0.88–0.96。S6（歪み）とS7（位置+スケール）は0.91–0.95のカバレッジを達成
3. **精度**: $n \geq 100$ でRMSEが0.12未満、CI幅が0.38未満
4. **感度**: SMDが位置のみに限定されるのに対し、位置・スケール・歪度の差を捕捉

シナリオS3（$0.5\sigma$ の位置シフト、真のnABCD $= 0.358$）は、臨床的に関連する効果量での操作特性を例示する：バイアスは無視できる（$n = 100$ で $+0.009$）、カバレッジは名目どおり（0.945）、CI幅（0.36）はGUSTO-I適用における年齢を代表する $L = 0.01$/yr、IQR $= 17$ で較正すると $0.01 \times 17 \times 0.36 = 0.061$（6.1\%pt）の $\Delta_{\max}$ CI幅に変換され——情報に基づく規制審議に十分な精度水準である。
...
![図\ref{fig:gusto_forest}: Region~8 対 各パートナー地域のnABCDフォレストプロット](../figures/fig4_gusto_r8_forest.png)
...
2つの候補効果修飾因子は、異なる分布類似性のパターンを示す。年齢については、パートナー地域のnABCD点推定値は狭い範囲にわたっており、下端の0.022（R5、R7）から上端の0.151（R3）までであり、15のパートナーのうち11が年齢nABCD 0.080未満である。SBPについては、点推定値はより広い範囲にわたり、0.030（R2）から0.219（R9）までで、大半のパートナーは0.100–0.220の区間に収まる。したがってGUSTO-IにおけるSBP分布は年齢分布よりも地域間異質性が大きく、候補効果修飾因子ごとのパートナー順位はそれに応じて異なる。

各ランキングの中央付近には、不確実性区間が大きく重なり合うパートナーがいくつか存在する。R16の年齢nABCD（$0.101$）は13番目に小さい値で95% CI $[0.067, 0.143]$ を持ち、相対的順位がデータによって鋭く決まらない領域に位置している——このCIは、より小さな点推定値を持ついくつかのパートナーがカバーする範囲にまで及んでいる。R6の年齢nABCD（$0.053$）は9番目に小さい値であるが、SBP nABCD 0.099はSBP-nABCD分布の下端付近に位置し、95% CI $[0.063, 0.152]$ は同様にSBPランキングの広い領域にまたがる。これらの中位ランクのケースは、点推定値とともにブートストラップCIを報告することの価値を示している：ランキング自体に不確実性区間が伴っており、あるパートナーを別のパートナーの上または下にどれほど確信をもって配置できるかを示す情報を与える。

R2とR9の間には示唆的な対比が現れる：R2は2番目に大きい年齢nABCD（0.122）を持つが最小のSBP nABCD（0.030）を持ち、一方R9は4番目に小さい年齢nABCD（0.033）を持つが最大のSBP nABCD（0.219）を持つ。いずれか一方の候補効果修飾因子のみに基づくランキングは、この2地域について正反対の結論に至るだろう。この非対称性は、図\ref{fig:gusto_scatter}で視覚化するように、すべての候補効果修飾因子を同時に評価することの必要性を強調している。

![図\ref{fig:gusto_scatter}: Region~8に対する年齢×SBP同時分布評価散布図](../figures/fig5_gusto_r8_scatter.png)

> **図\ref{fig:gusto_scatter}**: Region~8に対する各パートナー地域のnABCD点推定値の同時分布。ラベル付きの各点は1つのパートナーを表し、年齢に対するnABCD（横軸）とSBPに対するnABCD（縦軸）で配置される。両軸は同じスケールを用いる。

### 4.3 臨床的解釈とプーリング候補

各候補効果修飾因子について、CATE感度 $L$ の上界を要請する；ここで $L$ は、治療効果が修飾因子に応じてどれほど強く変動するかを支配することを想起されたい。急性心筋梗塞における血栓溶解療法について、クラスレベルのメタ解析エビデンス（GUSTO-I (GUSTO Investigators 1993)、FTT (FTT 1994)）は、年齢関連およびSBP関連の30日死亡率に対する治療効果修飾が、それぞれおおむね10年あたり10\%pt以内、10~mmHgあたり2\%pt以内に収まることを示唆する。これらは中心的な効果推定値ではなく、妥当性の上限エンベロープ（upper plausibility envelope）として意図されたものである。本論文ではこれらを例示的な上界として採用する：$L_{\text{age}} = 1\times10^{-2}$ /yr および $L_{\text{SBP}} = 2\times10^{-3}$ /mmHg。

各パートナー地域について、地域別の $L^*$ 値（表\ref{tab:gusto_lstar_joint}）を対応する妥当な上界（$L_{\text{UB}}$ と表記）と比較する。ある効果修飾因子についてある地域がプーリングに適格（eligible）と判定されるのは $L^* > L_{\text{UB}}$ の場合であり、これは観察された分布差が $\Delta_{\text{clin}} = 1$\%pt を生じるために必要なCATE感度が妥当な上界を超えることを意味する。年齢については、適格パートナーはR1、R4、R5、R6、R7、R9、R12、R14、R15（$L^*_{\text{age}} > 0.01$ となる9地域）である。SBPについては、適格パートナーはR1、R4、R5、R6、R13、R14、R15（$L^*_{\text{SBP}} > 0.002$ となる7地域）である；R7（$L^*_{\text{SBP}} = 0.0020 = L_{\text{UB}}$）は除外される。

**表\ref{tab:gusto_lstar_joint}: 15のパートナー地域すべてに対する臨床的解釈（$\Delta_{\text{clin}} = 1$\%pt における $L^*$）と同時適格性、地域番号順。$L^*$ は、観察された分布差を絶対30日死亡率スケール上の $1$\%pt の治療効果差に変換するために必要な最小CATE感度（年齢は年あたり、SBPはmmHgあたり）である。適格性は $L^* > L_{\text{UB}}$ で判定し、$L_{\text{age},\text{UB}} = 1\times10^{-2}$ /yr および $L_{\text{SBP},\text{UB}} = 2\times10^{-3}$ /mmHg を用いる。**

| パートナー | nABCD\textsubscript{age} | nABCD\textsubscript{SBP} | $L^*$ 年齢 (/yr) | $L^*$ SBP (/mmHg) | 年齢適格 | SBP適格 | 同時適格 |
|-----------|-------------------------:|-------------------------:|------------------:|-------------------:|:--------:|:-------:|:--------:|
| R1  | 0.035 | 0.116 | 0.0155 | 0.0028 | ✓ | ✓ | ✓ |
| R2  | 0.122 | 0.030 | 0.0047 | 0.0105 | --- | ✓ | --- |
| R3  | 0.151 | 0.102 | 0.0038 | 0.0030 | --- | ✓ | --- |
| R4  | 0.031 | 0.084 | 0.0180 | 0.0038 | ✓ | ✓ | ✓ |
| R5  | 0.022 | 0.112 | 0.0256 | 0.0030 | ✓ | ✓ | ✓ |
| R6  | 0.053 | 0.099 | 0.0110 | 0.0030 | ✓ | ✓ | ✓ |
| R7  | 0.022 | 0.164 | 0.0247 | 0.0020 | ✓ | --- | --- |
| R9  | 0.033 | 0.219 | 0.0171 | 0.0015 | ✓ | --- | --- |
| R10 | 0.076 | 0.125 | 0.0072 | 0.0025 | --- | ✓ | --- |
| R11 | 0.070 | 0.208 | 0.0079 | 0.0016 | --- | --- | --- |
| R12 | 0.048 | 0.194 | 0.0116 | 0.0016 | ✓ | --- | --- |
| R13 | 0.067 | 0.075 | 0.0087 | 0.0042 | --- | ✓ | --- |
| R14 | 0.040 | 0.128 | 0.0142 | 0.0023 | ✓ | ✓ | ✓ |
| R15 | 0.034 | 0.148 | 0.0164 | 0.0021 | ✓ | ✓ | ✓ |
| R16 | 0.101 | 0.143 | 0.0057 | 0.0023 | --- | ✓ | --- |

> 地域は地域番号順に記載しており、この順序に暗黙のランキングはない。✓ は適格（$L^* > L_{\text{UB}}$）を、--- は不適格を示す。

ある地域が同時適格（jointly eligible）であるとは、両候補効果修飾因子について基準を満たす場合をいう。プーリングは評価対象のすべての修飾因子について分布的整合性を要請するためである。このAND条件を満たすのは6地域：R1、R4、R5、R6、R14、R15 である。残る9パートナーは少なくとも一方の効果修飾因子について不適格である——例えばR2は年齢で不適格（$L^*_{\text{age}} = 0.0047$）、R9はSBPで不適格（$L^*_{\text{SBP}} = 0.0015$）であり、これらの臨床的仮定の下ではプーリングは推奨されない。

同時適格パートナーの中で、R4が単一プーリング候補（single-pool candidate）として最も有力な候補として浮上する。R4は年齢で3番目に低く（0.031）、SBPで4番目に低く（0.084）、両修飾因子にわたってバランスがとれている。R5は年齢nABCDが最低（0.022）であるが、SBP nABCDは6番目に低い（0.112）にとどまり、非対称な profile を示す。本フレームワークは、スポンサーが臨床的・規制的助言者と協議する際の判断を支援する定量的入力——表\ref{tab:gusto_nabcd}のブートストラップCIつきnABCD、表\ref{tab:gusto_lstar_joint}のパートナー別 $L^*$、および仮定された上界の下での同時適格性——を供給する。なお、GUSTO-Iデータは1990–1993年に収集されたものであり、本適用事例は、公開IPDを用いた方法論的例示として提示するものであって、これら特定の分布を現代の血栓溶解薬試験計画に対する根拠として推奨するものではない。
...
nABCDは包括的な規制評価における一つの入力に過ぎず、生物学的妥当性、臨床的関連性、ベネフィット・リスクバランスは、いかなる単一の分布測度も定量化できる範囲を超える。\cite{iche17,matsushima2024} 本フレームワークにはさらに以下の限界がある。連続型効果修飾因子を周辺的に扱うため、カテゴリカルおよび多変量への拡張は今後の課題である。Wasserstein距離の非負関数として、nABCDは真値がゼロまたはその近傍にある場合に正のバイアスを示し、ブートストラップカバレッジはゼロとなる（S1；第\ref{sec:sim_results}節）\cite{andrews2000} ため、各地域 $n \geq 100$ を推奨し、帰無近傍の推定値は注意して解釈すべきである。臨床較正はCATE感度 $L$ に依存するが、$L$ は新規薬剤に対して利用不可能でありうる（$L^*$ 経路で対処）か、先行エビデンスから繰り越される（臨床的仮定を伴う）。本フレームワークは定量的入力を提供するがカットオフは規定しない。最後に、共変量に基づくアプローチ全般と同様に、測定されていない効果修飾因子は指標の捕捉範囲を超えて異質性に寄与しうる。
...
非負性はWasserstein-1距離の非負性から直接従う：$W_1(F_1, F_2) = \int |F_1(x) - F_2(x)| \, dx \geq 0$ であり、すべての $x$ について $F_1(x) = F_2(x)$ の場合にのみ等号が成立する。非退化分布に対して $\text{IQR}_{\text{pooled}} > 0$ であるから、比率 $\text{nABCD} = W_1 / \text{IQR}_{\text{pooled}} \geq 0$ である。
...
一次元では、経験的 $W_1$ 距離は対数修正なしに $O(n^{-1/2})$ のレートで収束する (del Barrio et al. 1999)。$F_1 \neq F_2$ の二標本推定で有限二次モーメントのもと、$\sqrt{n}(W_1(\hat{F}_{1,n_1}, \hat{F}_{2,n_2}) - W_1(F_1, F_2))$ は平均ゼロのガウス分布に分布収束する（$n = n_1 + n_2$、$n_1/n \to \lambda \in (0,1)$）。経験的IQRも標準的な密度条件のもとで $O(n^{-1/2})$ のレートで収束するため、比率 $g(w, q) = w/q$ に対するデルタ法により $\widehat{\text{nABCD}}$ の漸近正規性が得られる：
